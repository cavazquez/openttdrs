...
> Corte V1: 2026-09-20, candidata actual de `main`. ✅ significa que pasa
...
| Render OpenGFX vanilla | Sprites y compositor amplios; framebuffer global todavía divergente | ✅ | [#589](https://github.com/cavazquez/openttdrs/issues/589) certifica Kale `(132,2)` Normal: 45/480 píxeles, 5/24 >64, media 0,00244/0,05 y cobertura 0 ([evidencia](docs/parity/raster-v1-kale.md)); los otros zooms siguen diagnósticos y el cierre espera CI remota vigente |
...
| Barcos | Depósitos, docks, boyas, esclusas y controlador naval | ✅ | [#593](https://github.com/cavazquez/openttdrs/issues/593) certifica depósito → muelle de carga → boya → entrega pagada de carbón, con conservación, dos corridas idénticas y hash canónico ([evidencia](docs/parity/ship-delivery-v1.md)); quedan fuera canales/locks/YAPF global y espera CI remota vigente antes del cierre |
| Aviones | FTA, compra/vuelo y oracle Helidepot (4 tests verificados) | ✅ | [#594](https://github.com/cavazquez/openttdrs/issues/594) certifica Dakota Country → Country: carga, despegue, FTA, aterrizaje y pago, con reservas sin huérfanos, dos corridas idénticas y hash canónico ([evidencia](docs/parity/air-delivery-v1.md)); no todos los layouts y espera CI remota vigente antes del cierre |
| Multijugador propio | TCP lockstep, dedicated, late join, resync y host migration | ✅ | Gate TCP obligatorio [#585](https://github.com/cavazquez/openttdrs/issues/585) ✅ (`PermissionDenied` falla; inyección negativa + 22 recorridos loopback, 0 omitidas). [#595](https://github.com/cavazquez/openttdrs/issues/595) certifica dedicated + dos clientes reales: el segundo entra en tick 500 y los 2.000 ticks conservan hash, logs de comandos e issuers sin diferencias ([evidencia](docs/parity/late-join-v1.md)); queda CI remota vigente antes del cierre. Sin protocolo OpenTTD |
...
| GS-lite propio | Goals, story y league; `CargoDelivered` cuenta sólo las unidades finales del tipo solicitado por la compañía | 🟡 | [#598](https://github.com/cavazquez/openttdrs/issues/598) y [#599](https://github.com/cavazquez/openttdrs/issues/599) implementados: ledger persistente por cargo y meta `Coal=10` que guarda en 5/10, continúa 2.000 ticks y conserva hash/noticia única ([evidencia](docs/parity/gs-json-v1.md)); ambos esperan CI remota vigente. No GameScript/Squirrel |
| Editor de escenarios | Herramientas, guardado y apertura propios | ✅ | [#600](https://github.com/cavazquez/openttdrs/issues/600) certifica un Temperate 64×64: elevar una tesela, pueblo, mina y central; error de guardado sin cerrar el documento, JSON aislado y reapertura por Escenarios con mapa, entidades, fecha/settings y tick idénticos ([evidencia](docs/parity/editor-v1.md)). No `.scn` universal; espera CI remota vigente antes del cierre |
| Distribución alpha | Prerelease multiplataforma publicada; fix y smoke Snap read-only locales | 🟡 | El [smoke Snap](docs/parity/snap-v1.md) preserva SHA/logs/captura para [#582](https://github.com/cavazquez/openttdrs/issues/582)/[#583](https://github.com/cavazquez/openttdrs/issues/583), pero falta instalar/validar una revisión de tienda; el [gate gráfico nativo](docs/parity/package-native-graphical-v1.md) ya prepara ZIP Windows [#601](https://github.com/cavazquez/openttdrs/issues/601) y tar.gz macOS [#602](https://github.com/cavazquez/openttdrs/issues/602), pero faltan sus dry-runs en sesiones gráficas reales |
| Certificación del corte | Tests y reportes parciales existentes | 🟡 | El [agregador fail-closed](docs/parity/v1-report.md) de [#603](https://github.com/cavazquez/openttdrs/issues/603) ya rechaza ausencias, SHA mezclados, `skip` y umbrales incumplidos; faltan los 20 resultados verificables del mismo candidato |
