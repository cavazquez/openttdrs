...
fn spawn_newgrf_station_layout_ground(
    ...
    if let Some(parent) = foundation_child_parent {
        ...
        // `DrawStationTile` materializa el ground de TileLayout con
        // `DrawGroundSprite`; sólo la secuencia BUILD entra al compositor
        // global. En pendiente la fundación ya conserva el anclaje como child.
        position.z = ground_draw_z(ctx.tx_i32(), ctx.ty_i32(), 0.025);
        commands.spawn((
            MapVisualLayer,
            ctx.map_tile_chunk(),
            sprite,
...
fn spawn_newgrf_road_stop_layout_ground(
    ...
    if let Some(parent) = foundation_child_parent {
        ...
        // `DrawGroundSprite` conserva el ancla NFO, pero no participa del
        // stream de parents. Separarlo del pase sortable evita que el asfalto
        // custom cubra BUILD/catenaria que OpenTTD dibuja después.
        position.z = ground_draw_z(ctx.tx_i32(), ctx.ty_i32(), 0.025);
        commands.spawn((
            MapVisualLayer,
            ctx.map_tile_chunk(),
...
fn spawn_newgrf_airport_layout_ground(
    ...
    if let Some(parent) = foundation_child_parent {
        ...
        // `AirportDrawTileLayout` entrega su ground por `DrawGroundSprite`.
        // Mantenerlo fuera del stream sortable evita que tape un BUILD de la
        // misma fila diagonal; una foundation conserva el vínculo child.
        position.z = ground_draw_z(ctx.tx_i32(), ctx.ty_i32(), 0.025);
        commands.spawn((
            MapVisualLayer,
            ctx.map_tile_chunk(),
...
mod tests {
    ...
    fn newgrf_tile_layout_orphan_child_uses_ground_screen_offsets() {
        ...
        assert_eq!(center.x, base.x + 7.0);
        assert_eq!(center.y, base.y + 4.0);
        assert_eq!(center.z, crate::iso::sortable_draw_z(3, 4, 0, 0.05));
    }
