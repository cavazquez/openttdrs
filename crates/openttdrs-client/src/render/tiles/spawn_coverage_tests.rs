...
#[test]
fn level_crossing_draws_road_catenary() {
    let assets = boot_assets_app();
    let coord = TileCoord::new(3, 3);
    let mut map = fresh_map8();
    let crossing = Tile {
        kind: TileKind::Road,
        mapt: 0x20,
        // `RoadTileType::Crossing`, road axis X → ROAD_X (0x0A).
        m5: 0x40,
        ..tile_template()
    };
    map.set_tile(coord, crossing)
        .expect("cruce vial electrificado");
    ...
    world
        ...
        .expect("cruce vial con catenaria");
    ...
    assert_eq!(
        catenary_parents, 4,
        "DrawRoadCatenary también debe ejecutarse en cruces a nivel"
    );
}
...
#[test]
fn river_ship_depot_consumes_all_slope_specific_river_edge_blocks() {
    for (tileh, slope_index, elevated_corners, edge_offset) in [
        (SLOPE_SE, 0, [(1, 2), (2, 2)], 12),
        (SLOPE_NE, 1, [(1, 1), (1, 2)], 24),
        (SLOPE_SW, 2, [(2, 1), (2, 2)], 36),
        (SLOPE_NW, 3, [(1, 1), (2, 1)], 48),
    ] {
        let assets = boot_assets_app();
        let depot = TileCoord::new(1, 1);
        let mut map = Map::new_flat(4, 4, 0);
        ...
        features[usize::from(openttdrs_core::CF_RIVER_SLOPE)].newgrf_views = (0..4)
            ...
            .collect();
        // Cada pendiente ocupa un bloque de 12 sprites en CF_RIVER_EDGE.
        features[usize::from(openttdrs_core::CF_RIVER_EDGE)].newgrf_views = (0..60)
        ...
        world
            .run_system_once(
                move |mut commands: Commands,
                      ...
                      mut action5_sprites: Local<crate::render::NewGrfAction5SpriteCache>,
                      mut images: ResMut<Assets<Image>>| {
                    let ctx = TileRenderContext::new(&m.0, &g.0, 1, 1);
                    assert_eq!(ctx.info.tileh, tileh, "se resolvió la pendiente {tileh}");
                    spawn_transport_object_tile_with_road_types_and_tramway_action5(
                        &mut commands,
                        &a.0,
        ...
        assert_eq!(
            markers
                .iter()
                .filter(|&&marker| marker == 0x60 + slope_index)
                .count(),
            1
        );
        for slot in edge_offset..edge_offset + 8 {
            let marker = 0x80 + slot as u8;
            assert_eq!(
                markers.iter().filter(|&&value| value == marker).count(),
                1,
                "el borde River usa el bloque de la pendiente {tileh}, slot {slot}"
            );
        }
        assert_eq!(
            world.query::<&MapVisualLayer>().iter(&world).count(),
...
#[test]
fn river_ship_depot_applies_sprite_offset_callback_to_ground_and_edges() {
    let assets = boot_assets_app();
    let depot = TileCoord::new(1, 1);
    let mut map = Map::new_flat(4, 4, 0);
    let mut tile = tile_template();
    ...
    features[usize::from(openttdrs_core::CF_RIVER_SLOPE)] = openttdrs_core::CanalFeatureDef {
        ...
        from_newgrf: true,
        grfid: 0xCAFE,
        newgrf_views: (0..6).map(|slot| marked_sprite(0x40 + slot)).collect(),
    ...
    features[usize::from(openttdrs_core::CF_RIVER_EDGE)] = openttdrs_core::CanalFeatureDef {
        ...
        from_newgrf: true,
        grfid: 0xCAFE,
        newgrf_views: (0..40).map(|slot| marked_sprite(0x80 + slot)).collect(),
        newgrf_runtime: Some(Box::new(callback_literal_runtime(
    ...
    world
        ...
        .expect("river ship depot callback spawn");
    ...
    let sprites: Vec<_> = world
        ...
        .collect();
    assert_eq!(sprites.len(), 9, "ground River y ocho bordes con callback");
    assert_eq!(sprites.iter().filter(|&&marker| marker == 0x43).count(), 1);
    for slot in 26..34 {
        let marker = 0x80 + slot as u8;
        assert_eq!(
            sprites.iter().filter(|&&value| value == marker).count(),
            1,
            "el callback River conserva el bloque y desplaza el slot {slot}"
        );
    }
    assert_eq!(
        world.query::<&MapVisualLayer>().iter(&world).count(),
        10,
        "el callback River no pierde la estructura del depósito"
    );
}

#[test]
fn river_ship_depot_callback_preserves_concave_neighbor_edge_selection() {
    let assets = boot_assets_app();
    let depot = TileCoord::new(2, 2);
    let mut map = Map::new_flat(5, 5, 0);
    let mut tile = tile_template();
    ...
    tile.m1 = set_water_class_m1(tile.m1, WaterClass::River);
    map.set_tile(depot, tile).expect("river ship depot");
    for coord in [TileCoord::new(1, 2), TileCoord::new(2, 3)] {
        make_water_tile(&mut map, coord, WaterClass::River).expect("river neighbour");
    }
    let grid = RenderGrid::from_map(&map, 5, 5);
    let marked_sprite = |marker: u8| DecodedSprite {
        width: 3,
    ...
    world
        ...
        .expect("river ship depot neighbour callback spawn");
    ...
    let sprites: Vec<_> = world
        ...
        .collect();
    // Sides 2/3, the outer corner 6 and the concave corner 8 remain. The
    // callback adds two to each requested CF_RIVER_EDGE slot.
    assert_eq!(sprites.len(), 4, "sólo quedan bordes River no compartidos");
    for marker in [0x84, 0x85, 0x88, 0x8A] {
        assert_eq!(
            sprites.iter().filter(|&&value| value == marker).count(),
            1,
            "el callback conserva la conectividad del slot {marker:#04x}"
        );
    }
    assert_eq!(
        world.query::<&MapVisualLayer>().iter(&world).count(),
        6,
        "los bordes cóncavos no pierden el ground ni la estructura"
    );
}
...
#[test]
fn newgrf_object_direct_water_ground_matches_water_classes() {
    use openttdrs_core::map::{MP_OBJECT_MAPT, SPR_FLAT_WATER_TILE};
    use openttdrs_core::newgrf_sprites::{TileLayout, TileLayoutSpriteRef};

    let object_type = openttdrs_core::NEW_OBJECT_OFFSET;
    let fallback_view = DecodedSprite {
        width: 2,
        height: 2,
        x_offs: -1,
        y_offs: -2,
    ...
    runtime.tile_layouts.insert(
        ...
        TileLayout {
            ground: TileLayoutSpriteRef {
                direct_sprite: SPR_FLAT_WATER_TILE as u16,
                ..Default::default()
            },
            sequence: Vec::new(),
        },
    ...
    for (water_class, expected_dikes) in [
        ...
        object_tile.m1 = set_water_class_m1(object_tile.m1, water_class);
        map.set_tile(coord, object_tile)
            .expect("object on direct water class");
        ...
        world
            ...
            .expect("object direct water ground spawn");

        assert_eq!(
            world.query::<&WaterTile>().iter(&world).count(),
            1,
            "{water_class:?} conserva una superficie desde el sprite directo"
        );
        let sprites: Vec<_> = world.query::<&Sprite>().iter(&world).collect();
        let dikes = dike_assets
            .iter()
        ...
        assert_eq!(
            dikes, expected_dikes,
            "diques del objeto con ground directo sobre {water_class:?}"
        );
        assert_eq!(
            world.query::<&MapVisualLayer>().iter(&world).count(),
            expected_dikes + 1,
            "ground y bordes del objeto con ground directo sobre {water_class:?}"
        );
        assert!(
            world
                .resource::<Assets<Image>>()
                .iter()
                .all(|(_, image)| image.data.as_deref() != Some(fallback_view.rgba.as_slice())),
            "el fallback rojo no reemplaza el ground directo de agua en {water_class:?}"
        );
    }
}

#[test]
fn newgrf_object_draw_water_applies_canal_feature_callback() {
    use openttdrs_core::map::MP_OBJECT_MAPT;
    use openttdrs_core::newgrf_sprites::{TileLayout, TileLayoutSpriteRef};
    ...
    let object_def = ObjectSpecDef {
        id: object_type,
        class_label: "TEST".into(),
        name: "Callback water object".into(),
        size: openttdrs_core::OBJECT_SIZE_1X1,
        from_newgrf: true,
        local_id: 4,
        grfid: 0x4F42_4A54,
    ...
    object_tile.m1 = set_water_class_m1(object_tile.m1, WaterClass::Canal);
    map.set_tile(coord, object_tile)
        .expect("object on callback canal");
    let grid = RenderGrid::from_map(&map, 3, 3);
    let mut world = World::new();
    world.insert_resource(TsMap(map));
    world.insert_resource(TsGrid(grid));
    ...
    world
        ...
        .expect("object canal callback spawn");
    ...
    assert!(
        ...
        "el ground del objeto usa slot 0 + delta del callback: {markers:?}"
    );
    for marker in 0x82..=0x89 {
        assert_eq!(
            markers.iter().filter(|&&value| value == marker).count(),
            1,
            "el dique del objeto conserva slot y delta: {marker:#04x}"
        );
    }
    assert_eq!(
        world.query::<&MapVisualLayer>().iter(&world).count(),
        9,
        "callback del objeto no pierde ground ni ocho diques"
    );
    assert!(
        world
            .resource::<Assets<Image>>()
        ...
        "el ground Action1 rojo no reemplaza la superficie Canal"
    );
}
...
#[test]
fn direct_house_ground_uses_the_house_atlas_and_ground_anchor() {
    let assets = boot_assets_app();
    let layer = openttdrs_core::newgrf_sprites::ResolvedTileLayoutSprite {
        sprite: None,
        base_sprite: Some(1424),
        sprite_modifiers: 0,
        direct_palette: 0,
        origin: [0, 0, 0],
        extent: [1, 1, 1],
    ...
    let resolved = direct_tile_layout_sequence(&layer, &assets).expect("house ground sprite");
    assert!(resolved.atlas.matches(&assets.houses[&1424].sprite()));
    assert_eq!(resolved.width, 64.0);
...
#[test]
fn direct_road_waypoint_build_uses_waypoint_atlas_and_nfo_geometry() {
    let assets = boot_assets_app();

    for axis in 0..2 {
        for layer in crate::sprites::road_waypoint_build_layers(axis) {
            let sprite_id = u16::try_from(layer.sprite_id).expect("waypoint SpriteID");
            let atlas_index = crate::sprites::road_waypoint_sprite_index(layer.sprite_id)
                .expect("waypoint atlas index");
            let resolved = direct_tile_layout_road_waypoint_sequence(
            ...
            .unwrap_or_else(|| panic!("road waypoint BUILD sprite {sprite_id}"));

            assert!(
                resolved
                    .atlas
                    .matches(&assets.road_waypoint[atlas_index].sprite()),
                "sprite {sprite_id}"
            );
            assert_eq!(resolved.width, layer.w, "sprite {sprite_id}");
