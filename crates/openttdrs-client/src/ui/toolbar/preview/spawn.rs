        ...
        PreviewPlan::BridgeSpan { tiles, valid } => {
            ...
            spawn_bridge_span_preview(
                ...
                BridgeSpanPreviewSpawn {
                    ...
                    map: &sim.state.map,
                    valid: *valid,
                    rail_type: sim.state.current_rail_type,
                    road_def,
        ...
        TilePreviewKind::RailDepot { dir, rail_type } => {
            spawn_rail_depot_preview(
                ...
                RailDepotPreviewSpawn {
                    ...
                    signal_sprites: &mut preview_newgrf.signal_sprites,
                },
            );
        }
...
fn spawn_rail_ghost_preview(commands: &mut Commands, atlas: &TileAtlas, spawn: RailGhostSpawn) {
    ...
    for (i, sid) in ids.iter().copied().enumerate() {
        ...
        // OpenTTD: fantasma de vía en rojo (válido más claro; inválido más oscuro).
        let tint = if spawn.valid {
            if is_overlay {
                Color::srgba(2.8, 0.55, 0.45, 0.95)
            } else {
                Color::srgba(1.0, 0.28, 0.22, 0.55)
            }
        } else if is_overlay {
            Color::srgba(2.2, 0.25, 0.2, 0.9)
        } else {
            Color::srgba(0.85, 0.12, 0.1, 0.5)
        };
