pub(super) const COAL_MINE_LAYOUTS: [&[(i32, i32, u8)]; 4] = [
    // OpenTTD _tile_table_coal_mine_0.
    &[
        (1, 1, 0),
        (1, 2, 2),
        (0, 0, 5),
        (1, 0, 6),
        (2, 0, 3),
        (2, 2, 3),
    ],
    // OpenTTD _tile_table_coal_mine_1.
    &[
        (1, 1, 0),
        (1, 2, 2),
        (2, 0, 0),
        (2, 1, 2),
        (1, 0, 3),
        (0, 0, 3),
        (0, 1, 4),
        (0, 2, 4),
        (2, 2, 4),
    ],
    // OpenTTD _tile_table_coal_mine_2.
    &[
        (0, 0, 0),
        (0, 1, 2),
        (0, 2, 5),
        (1, 0, 3),
        (1, 1, 3),
        (1, 2, 6),
    ],
    // OpenTTD _tile_table_coal_mine_3.
    &[
        (0, 1, 0),
        (0, 2, 2),
        (0, 3, 4),
        (1, 0, 5),
        (1, 1, 0),
        (1, 2, 2),
        (1, 3, 3),
        (2, 0, 6),
        (2, 1, 4),
        (2, 2, 3),
    ],
];

/// `OpenTTD` `_tile_table_power_station_0..=2` (gfx vanilla 7..=10).
pub(super) const POWER_STATION_LAYOUTS: [&[(i32, i32, u8)]; 3] = [
    &[
        (0, 0, 7),
        (0, 1, 9),
        (1, 0, 7),
        (1, 1, 8),
        (2, 0, 7),
        (2, 1, 8),
        (3, 0, 10),
        (3, 1, 10),
    ],
    &[
        (0, 1, 7),
        (0, 2, 7),
        (1, 0, 8),
        (1, 1, 8),
        (1, 2, 7),
        (2, 0, 9),
        (2, 1, 10),
        (2, 2, 9),
    ],
    &[
        (0, 0, 7),
        (0, 1, 7),
        (1, 0, 9),
        (1, 1, 8),
        (2, 0, 10),
        (2, 1, 9),
    ],
];

pub(super) const METAL_MINE_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_copper_mine_0.
    &[
        (0, 0, 47),
        (0, 1, 49),
        (0, 2, 51),
        (1, 0, 47),
        (1, 1, 49),
        (1, 2, 50),
        (2, 0, 51),
        (2, 1, 51),
    ],
    // OpenTTD _tile_table_copper_mine_1.
    &[
        (0, 0, 50),
        (0, 1, 47),
        (0, 2, 49),
        (1, 0, 47),
        (1, 1, 49),
        (1, 2, 51),
        (2, 0, 51),
        (2, 1, 47),
        (2, 2, 49),
    ],
];
pub(super) const GOLD_MINE_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_gold_mine_0.
    &[
        (0, 0, 72),
        (0, 1, 73),
        (0, 2, 74),
        (0, 3, 75),
        (1, 0, 76),
        (1, 1, 77),
        (1, 2, 78),
        (1, 3, 79),
        (2, 0, 80),
        (2, 1, 81),
        (2, 2, 82),
        (2, 3, 83),
        (3, 0, 84),
        (3, 1, 85),
        (3, 2, 86),
        (3, 3, 87),
    ],
];

pub(super) const BANK2_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_bank2_0.
    &[(0, 0, 89), (1, 0, 90)],
];

pub(super) const DIAMOND_MINE_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_diamond_mine_0.
    &[
        (0, 0, 91),
        (0, 1, 92),
        (0, 2, 93),
        (1, 0, 94),
        (1, 1, 95),
        (1, 2, 96),
        (2, 0, 97),
        (2, 1, 98),
        (2, 2, 99),
    ],
];
pub(super) const FOREST_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_forest_0.
    &[
        (0, 0, 16),
        (0, 1, 16),
        (0, 2, 16),
        (0, 3, 16),
        (1, 0, 16),
        (1, 1, 16),
        (1, 2, 16),
        (1, 3, 16),
        (2, 0, 16),
        (2, 1, 16),
        (2, 2, 16),
        (2, 3, 16),
        (3, 0, 16),
        (3, 1, 16),
        (3, 2, 16),
        (3, 3, 16),
        (1, 4, 16),
        (2, 4, 16),
    ],
    // OpenTTD _tile_table_forest_1.
    &[
        (0, 0, 16),
        (1, 0, 16),
        (2, 0, 16),
        (3, 0, 16),
        (4, 0, 16),
        (0, 1, 16),
        (1, 1, 16),
        (2, 1, 16),
        (3, 1, 16),
        (4, 1, 16),
        (0, 2, 16),
        (1, 2, 16),
        (2, 2, 16),
        (3, 2, 16),
        (4, 2, 16),
        (0, 3, 16),
        (1, 3, 16),
        (2, 3, 16),
        (3, 3, 16),
        (4, 3, 16),
        (1, 4, 16),
        (2, 4, 16),
        (3, 4, 16),
    ],
];
pub(super) const FARM_LAYOUTS: [&[(i32, i32, u8)]; 3] = [
    // OpenTTD _tile_table_farm_0.
    &[
        (1, 0, 33),
        (1, 1, 34),
        (1, 2, 36),
        (0, 0, 37),
        (0, 1, 37),
        (0, 2, 36),
        (2, 0, 35),
        (2, 1, 38),
        (2, 2, 38),
    ],
    // OpenTTD _tile_table_farm_1.
    &[
        (1, 1, 33),
        (1, 2, 34),
        (0, 0, 35),
        (0, 1, 36),
        (0, 2, 36),
        (0, 3, 35),
        (1, 0, 37),
        (1, 3, 38),
        (2, 0, 37),
        (2, 1, 37),
        (2, 2, 38),
        (2, 3, 38),
    ],
    // OpenTTD _tile_table_farm_2.
    &[
        (2, 0, 33),
        (2, 1, 34),
        (0, 0, 36),
        (0, 1, 36),
        (0, 2, 37),
        (0, 3, 37),
        (1, 0, 35),
        (1, 1, 38),
        (1, 2, 38),
        (1, 3, 37),
        (2, 2, 37),
        (2, 3, 35),
    ],
];
pub(super) const OIL_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_oil_well_0.
    &[(0, 0, 29), (1, 0, 29), (2, 0, 29), (0, 1, 29), (0, 2, 29)],
    // OpenTTD _tile_table_oil_well_1.
    &[(0, 0, 29), (1, 0, 29), (1, 1, 29), (2, 2, 29), (2, 3, 29)],
];

/// `OpenTTD` `_tile_table_oil_rig_0` (`IT_OIL_RIG`). La tesela 24 pasa a ser
/// `StationType::Oilrig` al terminar la construcción; las demás siguen como
/// industria y conservan la clase de agua de origen.
pub(super) const OIL_RIG_LAYOUTS: [&[(i32, i32, u8)]; 1] = [&[
    (0, 0, 24),
    (0, 1, 24),
    (0, 2, 25),
    (1, 0, 26),
    (1, 1, 27),
    (1, 2, 28),
]];

/// Las 52 entradas `GFX_WATERTILE_SPECIALCHECK` de `OpenTTD`
/// `_tile_table_oil_rig_0`.
///
/// No forman parte de la huella: sólo exigen agua plana durante la admisión y
/// deben seguir siendo agua tras materializar las seis teselas de la plataforma.
pub(super) const OIL_RIG_WATER_CHECKS: [(i32, i32); 52] = [
    (-4, -4),
    (-4, -3),
    (-4, -2),
    (-4, -1),
    (-4, 0),
    (-4, 1),
    (-4, 2),
    (-4, 3),
    (-4, 4),
    (-4, 5),
    (-4, 6),
    (-3, 6),
    (-2, 6),
    (-1, 6),
    (0, 6),
    (1, 6),
    (2, 6),
    (3, 6),
    (4, 6),
    (5, 6),
    (5, 5),
    (5, 4),
    (5, 3),
    (5, 2),
    (5, 1),
    (5, 0),
    (5, -1),
    (5, -2),
    (5, -3),
    (5, -4),
    (4, -4),
    (3, -4),
    (2, -4),
    (1, -4),
    (0, -4),
    (-1, -4),
    (-2, -4),
    (-3, -4),
    (2, 0),
    (2, -1),
    (1, -1),
    (0, -1),
    (-1, -1),
    (-1, 0),
    (-1, 1),
    (-1, 2),
    (-1, 3),
    (0, 3),
    (1, 3),
    (2, 3),
    (2, 2),
    (2, 1),
];
pub(super) const BANK_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_bank_0.
    &[(0, 0, 58), (1, 0, 59)],
];
pub(super) const REFINERY_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_oil_refinery_0.
    &[
        (0, 0, 20),
        (0, 1, 21),
        (0, 2, 22),
        (0, 3, 21),
        (1, 0, 20),
        (1, 1, 19),
        (1, 2, 22),
        (1, 3, 20),
        (2, 1, 18),
        (2, 2, 18),
        (2, 3, 18),
        (3, 2, 18),
        (3, 3, 18),
        (2, 0, 23),
        (3, 1, 23),
    ],
    // OpenTTD _tile_table_oil_refinery_1.
    &[
        (0, 0, 18),
        (0, 1, 18),
        (0, 2, 21),
        (0, 3, 22),
        (0, 4, 20),
        (1, 0, 18),
        (1, 1, 18),
        (1, 2, 19),
        (1, 3, 20),
        (2, 0, 18),
        (2, 1, 18),
        (2, 2, 19),
        (2, 3, 22),
        (1, 4, 23),
        (2, 4, 23),
    ],
];
pub(super) const FACTORY_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_factory_0.
    &[
        (0, 0, 39),
        (0, 1, 40),
        (1, 0, 41),
        (1, 1, 42),
        (0, 2, 39),
        (0, 3, 40),
        (1, 2, 41),
        (1, 3, 42),
        (2, 1, 39),
        (2, 2, 40),
        (3, 1, 41),
        (3, 2, 42),
    ],
    // OpenTTD _tile_table_factory_1.
    &[
        (0, 0, 39),
        (0, 1, 40),
        (1, 0, 41),
        (1, 1, 42),
        (2, 0, 39),
        (2, 1, 40),
        (3, 0, 41),
        (3, 1, 42),
        (1, 2, 39),
        (1, 3, 40),
        (2, 2, 41),
        (2, 3, 42),
    ],
];

pub(super) const PRINTING_WORKS_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_printing_works_0.
    &[
        (0, 0, 43),
        (0, 1, 44),
        (1, 0, 45),
        (1, 1, 46),
        (0, 2, 43),
        (0, 3, 44),
        (1, 2, 45),
        (1, 3, 46),
        (2, 1, 43),
        (2, 2, 44),
        (3, 1, 45),
        (3, 2, 46),
    ],
    // OpenTTD _tile_table_printing_works_1.
    &[
        (0, 0, 43),
        (0, 1, 44),
        (1, 0, 45),
        (1, 1, 46),
        (2, 0, 43),
        (2, 1, 44),
        (3, 0, 45),
        (3, 1, 46),
        (1, 2, 43),
        (1, 3, 44),
        (2, 2, 45),
        (2, 3, 46),
    ],
];

pub(super) const FOOD_PROCESS_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_food_process_0.
    &[
        (0, 0, 60),
        (1, 0, 60),
        (2, 0, 60),
        (0, 1, 60),
        (1, 1, 60),
        (2, 1, 60),
        (0, 2, 61),
        (1, 2, 61),
        (2, 2, 63),
        (0, 3, 62),
        (1, 3, 62),
        (2, 3, 63),
    ],
    // OpenTTD _tile_table_food_process_1.
    &[
        (0, 0, 61),
        (1, 0, 60),
        (2, 0, 61),
        (3, 0, 61),
        (0, 1, 62),
        (1, 1, 63),
        (2, 1, 63),
        (3, 1, 63),
        (0, 2, 60),
        (1, 2, 60),
        (2, 2, 60),
        (3, 2, 60),
        (0, 3, 62),
        (1, 3, 62),
    ],
];

pub(super) const PAPER_MILL_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_paper_mill_0.
    &[
        (0, 0, 64),
        (1, 0, 65),
        (2, 0, 66),
        (3, 0, 67),
        (0, 1, 68),
        (1, 1, 69),
        (2, 1, 67),
        (3, 1, 67),
        (0, 2, 66),
        (1, 2, 71),
        (2, 2, 71),
        (3, 2, 70),
    ],
];

pub(super) const STEEL_MILL_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_steel_mill_0.
    &[
        (2, 1, 52),
        (2, 2, 53),
        (3, 1, 54),
        (3, 2, 55),
        (0, 0, 56),
        (1, 0, 57),
        (0, 1, 56),
        (1, 1, 57),
        (0, 2, 56),
        (1, 2, 57),
        (2, 0, 56),
        (3, 0, 57),
    ],
    // OpenTTD _tile_table_steel_mill_1.
    &[
        (0, 0, 52),
        (0, 1, 53),
        (1, 0, 54),
        (1, 1, 55),
        (2, 0, 52),
        (2, 1, 53),
        (3, 0, 54),
        (3, 1, 55),
        (0, 2, 56),
        (1, 2, 57),
        (2, 2, 56),
        (3, 2, 57),
        (1, 3, 56),
        (2, 3, 57),
    ],
];
pub(super) const SAWMILL_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_sawmill_0.
    &[
        (1, 0, 14),
        (1, 1, 12),
        (1, 2, 11),
        (2, 0, 14),
        (2, 1, 13),
        (0, 0, 15),
        (0, 1, 15),
        (0, 2, 12),
    ],
    // OpenTTD _tile_table_sawmill_1.
    &[
        (0, 0, 15),
        (0, 1, 11),
        (0, 2, 14),
        (1, 0, 15),
        (1, 1, 13),
        (1, 2, 12),
        (2, 0, 11),
        (2, 1, 13),
    ],
];
pub(super) const IRON_MINE_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_iron_mine_0.
    &[
        (0, 0, 100),
        (0, 1, 101),
        (0, 2, 102),
        (0, 3, 103),
        (1, 0, 104),
        (1, 1, 105),
        (1, 2, 106),
        (1, 3, 107),
        (2, 0, 108),
        (2, 1, 109),
        (2, 2, 110),
        (2, 3, 111),
        (3, 0, 112),
        (3, 1, 113),
        (3, 2, 114),
        (3, 3, 115),
    ],
];

pub(super) const FRUIT_PLANTATION_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_fruit_plantation_0.
    &[
        (0, 0, 116),
        (0, 1, 116),
        (0, 2, 116),
        (0, 3, 116),
        (1, 0, 116),
        (1, 1, 116),
        (1, 2, 116),
        (1, 3, 116),
        (2, 0, 116),
        (2, 1, 116),
        (2, 2, 116),
        (2, 3, 116),
        (3, 0, 116),
        (3, 1, 116),
        (3, 2, 116),
        (3, 3, 116),
        (4, 0, 116),
        (4, 1, 116),
        (4, 2, 116),
        (4, 3, 116),
    ],
];

pub(super) const RUBBER_PLANTATION_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_rubber_plantation_0.
    &[
        (0, 0, 117),
        (0, 1, 117),
        (0, 2, 117),
        (0, 3, 117),
        (1, 0, 117),
        (1, 1, 117),
        (1, 2, 117),
        (1, 3, 117),
        (2, 0, 117),
        (2, 1, 117),
        (2, 2, 117),
        (2, 3, 117),
        (3, 0, 117),
        (3, 1, 117),
        (3, 2, 117),
        (3, 3, 117),
        (4, 0, 117),
        (4, 1, 117),
        (4, 2, 117),
        (4, 3, 117),
    ],
];

pub(super) const WATER_SUPPLY_LAYOUTS: [&[(i32, i32, u8)]; 1] =
    [&[(0, 0, 118), (0, 1, 119), (1, 0, 118), (1, 1, 119)]];

pub(super) const WATER_TOWER_LAYOUTS: [&[(i32, i32, u8)]; 1] = [&[(0, 0, 120)]];

pub(super) const FACTORY_TROPIC_LAYOUTS: [&[(i32, i32, u8)]; 2] = [
    // OpenTTD _tile_table_factory2_0.
    &[
        (0, 0, 121),
        (0, 1, 122),
        (1, 0, 123),
        (1, 1, 124),
        (0, 2, 121),
        (0, 3, 122),
        (1, 2, 123),
        (1, 3, 124),
    ],
    // OpenTTD _tile_table_factory2_1.
    &[
        (0, 0, 121),
        (0, 1, 122),
        (1, 0, 123),
        (1, 1, 124),
        (2, 0, 121),
        (2, 1, 122),
        (3, 0, 123),
        (3, 1, 124),
    ],
];

pub(super) const FARM_TROPIC_LAYOUTS: [&[(i32, i32, u8)]; 3] = [
    // OpenTTD _tile_table_farm2_0..=2 share the vanilla farm geometry.
    &[
        (1, 0, 33),
        (1, 1, 34),
        (1, 2, 36),
        (0, 0, 37),
        (0, 1, 37),
        (0, 2, 36),
        (2, 0, 35),
        (2, 1, 38),
        (2, 2, 38),
    ],
    &[
        (1, 1, 33),
        (1, 2, 34),
        (0, 0, 35),
        (0, 1, 36),
        (0, 2, 36),
        (0, 3, 35),
        (1, 0, 37),
        (1, 3, 38),
        (2, 0, 37),
        (2, 1, 37),
        (2, 2, 38),
        (2, 3, 38),
    ],
    &[
        (2, 0, 33),
        (2, 1, 34),
        (0, 0, 36),
        (0, 1, 36),
        (0, 2, 37),
        (0, 3, 37),
        (1, 0, 35),
        (1, 1, 38),
        (1, 2, 38),
        (1, 3, 37),
        (2, 2, 37),
        (2, 3, 35),
    ],
];

pub(super) const LUMBER_MILL_LAYOUTS: [&[(i32, i32, u8)]; 1] = [
    // OpenTTD _tile_table_lumber_mill_0.
    &[(0, 0, 125), (0, 1, 126), (1, 0, 127), (1, 1, 128)],
];
