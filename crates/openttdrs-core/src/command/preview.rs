...
fn preview_build_cmd(state: &GameState, cmd: &Command) -> Option<CommandError> {
    ...
    match cmd {
        ...
        Command::PlaceShipDepotDir(c, dir) => check_ship_depot_placement(state, *c, *dir)
            .err()
            .or_else(|| {
                crate::depot::next_free_depot_id(map)
                    .is_none()
        ...
        Command::PlaceDockAtStation {
            ...
            check_dock_placement_at_station_with_state(state, *origin, *dir, *station_to_join).err()
        }
        ...
        Command::PlaceAirportAreaWithLayout {
            ...
            check_airport_area_with_explicit_layout(state, *origin, *newgrf_spec_id, *layout, *spec)
                .err()
        }
        ...
        Command::ClearTile(c) => {
            ...
            if map.get(*c).is_some_and(|tile| {
                ...
                return check_clear_dock(state, *c).err();
            }
            if map.get_kind(*c) == Some(crate::map::TileKind::ShipDepot) {
                return check_clear_ship_depot(state, *c).err();
            }
            if map.get_kind(*c) == Some(crate::map::TileKind::Water) {
                return check_clear_water(state, *c).err();
            }
