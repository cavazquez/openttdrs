...
fn place_lock_clears_single_road_and_charges_road_clear() {
    ...
    apply_command(&mut s, &command).expect("una carretera simple es autoremovible");
...
#[test]
fn place_lock_clears_road_depot_and_unregisters_it() {
    let mut s = GameState::new(10, 6);
    ...
    apply_command(&mut s, &Command::PlaceRoad(entrance)).unwrap();
    apply_command(&mut s, &Command::PlaceRoadDepotDir(middle, 0)).unwrap();
    assert_eq!(s.map.get_kind(middle), Some(TileKind::RoadDepot));
    assert_eq!(s.depots.len(), 1);
    ...
    assert_eq!(command_would_fail(&s, &command), None);
    apply_command(&mut s, &command).expect("la demolición manual del depósito es válida");

    assert_eq!(s.map.get_kind(middle), Some(TileKind::Water));
    assert!(
        s.depots.is_empty(),
        "DEPT debe retirar el depósito demolido"
    );
    assert_eq!(s.map.get_kind(entrance), Some(TileKind::Road));
    assert_eq!(
        s.economy.money,
        money - road_depot_clear_cost(&s.global_economy) - lock_build_cost(&s.global_economy)
    );
}
...
#[test]
fn place_lock_clears_removable_object_without_autoremove() {
    let mut s = GameState::new(10, 6);
    let lower = TileCoord::new(2, 2);
    let middle = TileCoord::new(3, 2);
    let upper = TileCoord::new(4, 2);
    ...
    assert_eq!(command_would_fail(&s, &command), None);
    apply_command(&mut s, &command).expect("DoBuildLock limpia objetos removibles sin flag Auto");

    assert!(s.objects.is_empty(), "se quita la instancia completa");
    assert_eq!(s.random, expected_random);
    assert_eq!(s.map.get_kind(object_tiles[0]), Some(TileKind::Water));
    assert_eq!(
        s.economy.money,
        money
            - clear_cost
...
#[test]
fn place_lock_rejects_cannot_remove_object_atomically() {
    let mut s = GameState::new(10, 6);
    let lower = TileCoord::new(2, 2);
    let middle = TileCoord::new(3, 2);
    let upper = TileCoord::new(4, 2);
    let object_tiles = add_water_object(
        ...
        lower,
        0x11,
        crate::object_spec::OBJECT_FLAG_CANNOT_REMOVE,
    );
    for coord in [middle, upper] {
        s.map.set_kind(coord, TileKind::Water).unwrap();
    }
    ...
    assert_eq!(
        command_would_fail(&s, &command),
        Some(crate::CommandError::ObjectCannotBeRemoved)
    );
    assert_eq!(
        apply_command(&mut s, &command),
        Err(crate::CommandError::ObjectCannotBeRemoved)
    );
