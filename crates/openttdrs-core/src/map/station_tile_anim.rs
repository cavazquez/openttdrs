...
/// Sonido que `CB140`/`CB141`/`CB142` solicita al terminar un callback.
///
/// OpenTTD codifica el índice local en los bits 8..14 del resultado del
/// callback. Se mantiene separado del resultado visual para que las capas que
/// ejecutan la animación puedan encolarlo en el catálogo de sonidos del estado
/// sin cambiar las APIs históricas que solo devolvían teselas dirty.
...
///
/// Un sonido ausente o sin sample es silencioso, igual que el camino de audio
/// de OpenTTD cuando el efecto no está disponible en el catálogo cargado.
pub fn play_station_animation_sounds(
...
fn airport_tile_gfx(station: &Station, map: &Map, coord: TileCoord) -> Option<u16> {
    airport_tile_gfx_with_overrides(station, map, coord, &[])
}

fn airport_tile_gfx_with_overrides(
    station: &Station,
    map: &Map,
    coord: TileCoord,
...
fn airport_animation_context_with_towns_and_snow_line_and_overrides(
    map: &Map,
    stations: &[Station],
    ...
    let def = catalog
        ...
        .cloned()?;
    let mut ctx = crate::airport_tile_action2::action2_eval_ctx_for_airport_tile_with_towns_and_airport_catalog_and_snow_line_and_overrides(
        map,
        stations,
        towns,
        airport_catalog,
...
/// Variante de la animación de estación que aplica los overrides persistidos
/// al resolver `AirportTile` vecinos (`var 0x62`).
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_airport_animation_for_station_with_towns_and_cargo_catalog_and_airport_catalog_and_sounds_with_snow_line_and_overrides<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante global-RNG que aplica los overrides persistidos al resolver
/// `AirportTile` vecinos (`var 0x62`).
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_airport_animation_for_station_with_towns_and_cargo_catalog_and_airport_catalog_with_global_rng_and_sounds_with_snow_line_and_overrides<
    S: BuildHasher,
>(
    map: &mut Map,
    stations: &mut [Station],
...
>(
    ...
    step_newgrf_airport_tiles_with_towns_and_airport_catalog_and_sounds_with_snow_line_and_overrides(
        map,
        tick,
        stations,
        towns,
...
/// Variante del scheduler que aplica los overrides persistidos al scope de
/// cada `AirportTile` durante `TileLoop` y el avance periódico.
#[allow(clippy::too_many_arguments)]
pub fn step_newgrf_airport_tiles_with_towns_and_airport_catalog_and_sounds_with_snow_line_and_overrides<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
    ...
    for coord in candidates {
        let Some((_, def, _)) = airport_animation_context_with_towns_and_snow_line_and_overrides(
            map,
            stations,
            towns,
            catalog,
...
/// Variante del callback de una tesela que conserva también el pool de
/// pueblos para el `TownScopeResolver` parent de la estación.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_with_towns_and_world_and_cargo_catalog<S: BuildHasher>(
    map: &mut Map,
    tick: u64,
    stations: &mut [Station],
    companies: &[Company],
...
fn trigger_newgrf_station_animation_with_industries<S: BuildHasher>(
    ...
    trigger_newgrf_station_animation_with_industries_and_sounds(
        map,
        tick,
        stations,
        companies,
...
#[allow(clippy::too_many_arguments)]
fn trigger_newgrf_station_animation_with_industries_and_sounds<S: BuildHasher>(
    map: &mut Map,
    tick: u64,
    stations: &mut [Station],
    companies: &[Company],
...
/// Variante de la animación de una tesela que captura el sonido codificado por
/// el callback en `sound_events`.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_with_towns_and_world_and_cargo_catalog_and_sounds<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante `TA_WHOLE` que entrega también los pueblos al parent del scope.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_for_station_with_towns_and_world_and_cargo_catalog<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante `TA_WHOLE` que captura los sonidos codificados por cada callback.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_for_station_with_towns_and_world_and_cargo_catalog_and_sounds<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante `TA_PLATFORM` que entrega el pool de pueblos al scope parent.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_for_platform_with_towns_and_world_and_cargo_catalog<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante `TA_PLATFORM` que captura los sonidos codificados por cada callback.
#[allow(clippy::too_many_arguments)]
pub fn trigger_newgrf_station_animation_for_platform_with_towns_and_world_and_cargo_catalog_and_sounds<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
/// Variante del scheduler de estación que conserva pueblos para el scope
/// parent durante `TileLoop` y el avance de frames activos.
#[allow(clippy::too_many_arguments)]
pub fn step_newgrf_station_tiles_with_towns_and_world_and_cargo_catalog<S: BuildHasher>(
    map: &mut Map,
    tick: u64,
    stations: &mut [Station],
    companies: &[Company],
...
fn step_newgrf_station_tiles_with_industries<S: BuildHasher>(
    ...
    step_newgrf_station_tiles_with_industries_and_sounds(
        map,
        tick,
        stations,
        companies,
...
#[allow(clippy::too_many_arguments)]
fn step_newgrf_station_tiles_with_industries_and_sounds<S: BuildHasher>(
    map: &mut Map,
    tick: u64,
    stations: &mut [Station],
    companies: &[Company],
    ...
    let mut sound_events = sound_events;
    for (coord, _) in tile_loop_visits {
        if trigger_newgrf_station_animation_with_industries_and_sounds(
            map,
            tick,
            stations,
            companies,
...
/// Variante del scheduler que captura los sonidos de CB140/CB141/CB142.
#[allow(clippy::too_many_arguments)]
pub fn step_newgrf_station_tiles_with_towns_and_world_and_cargo_catalog_and_sounds<
    S: BuildHasher,
>(
    map: &mut Map,
    tick: u64,
...
mod tests {
    ...
    fn callback_literal(value: u16) -> Action2VarEntry {
        Action2VarEntry {
    ...
    fn station_animation_sound_callbacks() -> TrainSpriteGraphics {
        let mut gfx = station_animation_callbacks();
        // CB140 devuelve frame=5 y sound local=0x12 en bits 8..14.
        gfx.action2_var.insert(4, callback_literal(0x1205));
        // CB141 fija frame=3 + sound 0x13; CB142 espera 2 ticks + sound 0x14.
        gfx.action2_var.insert(5, callback_literal(0x1303));
        gfx.action2_var.insert(6, callback_literal(0x1402));
        gfx
    }
    ...
    #[test]
    fn station_animation_callback_sound_is_captured_and_played() {
        let coord = TileCoord::new(1, 1);
        let mut map = Map::new_flat(4, 4, 0);
        let mut tile = map.get(coord).unwrap();
        tile.kind = TileKind::Station;
        ...
        let mut stations = vec![Station::new_with_kind(coord, StopKind::RailStation)];
        let mut catalog = crate::vanilla_station_spec_catalog();
        let def = &mut catalog[0];
        def.from_newgrf = true;
        def.newgrf_grfid = 0x534E_4401;
        def.callback_mask = crate::STATION_CALLBACK_ANIMATION_NEXT_FRAME_MASK
            | crate::STATION_CALLBACK_ANIMATION_SPEED_MASK;
        def.animation_triggers = crate::STATION_ANIMATION_TRIGGER_BUILT;
        def.newgrf_runtime = Some(Box::new(station_animation_sound_callbacks()));
        let companies = vec![crate::Company::player(crate::CompanyEconomy::default(), 0)];
        let mut active = HashSet::new();
        let mut sounds = Vec::new();
        ...
        let visit_sounds = &mut sounds;
        let dirty = step_newgrf_station_tiles_with_towns_and_world_and_cargo_catalog_and_sounds(
            &mut map,
            4,
            &mut stations,
            &companies,
