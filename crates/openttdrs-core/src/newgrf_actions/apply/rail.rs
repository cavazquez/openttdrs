...
use crate::rail_type::{
    ...
    RAIL_SPRITE_TYPE_SIGNALS, RAIL_SPRITE_TYPE_TRACK_OVERLAY, RAIL_SPRITE_TYPE_TUNNEL,
    RAIL_SPRITE_TYPE_TUNNEL_PORTAL, RAIL_SPRITE_TYPE_UNDERLAY, RailSignalSpriteSpec, RailType,
...
pub fn apply_newgrf_rail_signals(state: &mut GameState, search_dirs: &[&Path]) {
    ...
    let mut tunnel_slots: Vec<Option<RailSignalSpriteSpec>> = vec![None; 4];
    let mut tunnel_portal_slots: Vec<Option<RailSignalSpriteSpec>> = vec![None; 4];
    ...
    for entry in &stack {
        ...
        for &(local_id, selector) in graphics.specific_assigns.keys() {
            ...
            match selector {
                ...
                RAIL_SPRITE_TYPE_GROUND | RAIL_SPRITE_TYPE_UNDERLAY => {
                ...
                RAIL_SPRITE_TYPE_TUNNEL => tunnel_slots[idx] = Some(spec),
                RAIL_SPRITE_TYPE_TUNNEL_PORTAL => tunnel_portal_slots[idx] = Some(spec),
                _ => {}
            }
        }
    }
    ...
    state.runtime.rail_type_tunnel_newgrf = tunnel_slots;
    state.runtime.rail_type_tunnel_portal_newgrf = tunnel_portal_slots;
