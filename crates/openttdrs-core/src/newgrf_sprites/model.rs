...
fn resolve_layout_sprite_asset(
    ...
    if let Some(set) = reference.action1_set {
        ...
        if direct_palette == PALETTE_NEWSPAPER {
            sprite.rgba = bake_sprite_newspaper(&sprite);
            // El remapeo ya quedó materializado; no debe reaplicarse sobre la
            // máscara de paleta al subir la textura al cliente.
            sprite.mask.clear();
            return Some((Some(sprite), None, 0));
...
/// Callback vehículos: solicitar efectos visuales avanzados (`0x160`).
...
/// Callback objetos: texto adicional de la ventana de fundación
/// (`CBID_OBJECT_FUND_MORE_TEXT`).
pub const CBID_OBJECT_FUND_MORE_TEXT: u16 = 0x15C;
