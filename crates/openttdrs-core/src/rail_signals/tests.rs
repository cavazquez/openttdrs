//! Tests de señales ferroviarias de bloque.

use super::presignal::{
    compute_exit_signal_greens, explore_sig_segment, presignal_exit_targets_ahead,
};
use super::*;
use crate::GameState;
use crate::map::{
    Map, RAIL_TB_HORZ, RAIL_TB_LOWER, RAIL_TB_UPPER, RAIL_TB_VERT, RAIL_TB_X, RAIL_TB_Y, TileCoord,
    TileKind,
};
...
fn write_signal_facing(map: &mut Map, c: TileCoord, tb: u8, face: Option<u8>) {
    ...
    let placement = signal_placement_for_track(track, face, 1, SIGTYPE_BLOCK).expect("placement");
    let mut t = map.get(c).expect("tile");
...
#[test]
fn indexed_globset_matches_full_update_and_scans_map_once() {
    use crate::Vehicle;
    use crate::vehicle::VehicleKind;

    let mut map = Map::new_flat(1_024, 4, 0);
    for x in 0..=8 {
        write_rail(&mut map, TileCoord::new(x, 1), RAIL_TB_X);
    }
    write_signal_facing(&mut map, TileCoord::new(2, 1), RAIL_TB_X, Some(0));
    ...
    let mut local = map;
    update_rail_signal_states(&mut local, &[], &mut Vec::new(), true);
    let mut index = SignalSpatialIndex::default();
    let mut glob = SignalGlobSet::new();
    enqueue_trains_for_signal_update(&mut glob, std::slice::from_ref(&blocker));
    ...
    enqueue_signal_glob(&mut glob, TileCoord::new(4, 1));
    drain_signal_globset_indexed_with_wormholes(
        &mut local,
        std::slice::from_ref(&blocker),
        &mut Vec::new(),
    ...
    assert_eq!(
        ...
        1,
        "el segundo drain no barre la grilla"
    );
    for x in [2, 6] {
        let c = TileCoord::new(x, 1);
        assert_eq!(
...
fn explore_sig_segment_flags_multi_exit_green() {
    ...
    let probe = explore_sig_segment(&map, TileCoord::new(1, 3), 0, None).with_green_flags(&greens);
...
fn dual_scenario_signal_9_controls_eastbound_exit() {
    use crate::parity::{TRAIN_DUAL_TRACK_OUT_Y, TRAIN_DUAL_VEHICLE_2_ID, build_train_supply_dual};
    ...
    let bits = signal_bits_for_exit(&state.map, sig, TileCoord::new(10, TRAIN_DUAL_TRACK_OUT_Y));
    assert_eq!(
        bits,
        vec![2],
        "m5={:#x} m3={:#x} m2={:#x}",
