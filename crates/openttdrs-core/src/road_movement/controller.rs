...
pub fn individual_road_vehicle_controller(
    ...
    if apply_road_veh_close_to(vehicles, v_idx) {
        return false;
    }
    ...
    let Some(rd) = road_drive_entry(v.road_state & 0x1F, next_frame) else {
        // Fin de tabla sin marcador: forzar NEXT_TILE lógico.
        return enter_next_tile(v, map);
    };
...
fn enter_next_tile(v: &mut Vehicle, map: Option<&Map>) -> bool {
...
mod tests {
    ...
    // `direction_from_trackdir` se usa en el módulo; mantener el import vivo en tests.
