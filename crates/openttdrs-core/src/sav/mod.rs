...
pub struct SavGame {
    ...
    /// Cadencia de crecimiento urbano (`economy.town_growth_rate` de `PATS` / `OPTS`).
...
pub fn load(raw: &[u8]) -> Result<SavGame, SavError> {
    ...
    Ok(SavGame {
        ...
        town_growth_rate: parsed_settings.town_growth_rate,
...
impl GameState {
    ...
    pub fn from_sav_game(mut sav: SavGame) -> Self {
        ...
        state.town_growth_rate = sav.town_growth_rate;
        ...
        for company in sav.companies {
            ...
            if let Some(target) = state.companies.get_mut(index) {
                ...
                if let Some(value) = company.inaugurated_year_calendar {
                    target.inaugurated_year_calendar = value;
                }
