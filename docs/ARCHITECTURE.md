...
principio incremental de este documento a una entrega vial verificable como
fixture interno. Las fases y prioridades históricas posteriores no reemplazan
ese corte.
...
6. **Sort del viewport** → `ViewportSortableParent::source_depth` es entrada y `Transform.z` es salida del sorter. Un sync visual debe comparar pose, visibilidad, `Sprite` y metadatos semánticos antes de escribir; en un frame estable conserva la Z resuelta, deja que el fast path omita el pase global y evita reextraer capas auxiliares idénticas.
7. **Assets visuales NewGRF** → Action2 se reevalúa para elegir la vista, pero la caché de `Image` se identifica por el resultado horneado final, no por el contexto de evaluación. En cada update la pila se resuelve una vez por entidad parent y sus children consumen ese resultado temporal; los offsets y dimensiones siguen siendo metadatos de cada capa.
