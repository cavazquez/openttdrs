...
El backlog ejecutable es #575–#577; #568–#574 entregaron los ajustes P0
iniciales, la ruta de carbón por comandos, su continuación JSON y la entrada
localizada desde el menú.
...
Corrección vigente del corte: `26a915db` conecta al runtime los campos
`INDY.accepted[].history`, `accepted[].accumulated_waiting`,
`INDY.produced[].history` y `valid_history` para cargos representables; las
entregas, transferencias, el barrido diario y el rollover mensual los
...
Actualización #329-INDTILE-ANIMATION-055 (2026-09-03, `c88518c4`):
`IndustryAnimationTrigger` separa los cinco eventos de `IndustryTile` de la
pasada visual. CB25 `TileLoop` sólo se ejecuta sobre visitas, `IndustryTick`
al intervalo de producción y `CargoReceived` después de confirmar la entrega;
CB26/CB27 avanzan únicamente la lista de teselas activas. La regresión cubre
...
Actualización #329-INDTILE-ANIMATION-057 (2026-09-03, `ca2939a7`):
`ConstructionStageChanged` queda conectado al alta inicial y a los cambios de
etapa observados por `TileLoop`. La primera llamada conserva el flag upstream
`var 18 |= 0x100`; las transiciones posteriores usan el ordinal sin extensión.
Ambos caminos hidratan el parent/PSA de la industria y tienen regresión del
...
GUI/variables ilimitadas y scopes económicos restantes siguen pendientes.

Actualización #329-VEHICLE-CUSTOM-REFIT-078 (2026-09-04, `61e0c53b`):
`CBID_VEHICLE_CUSTOM_REFIT` (`0x163`, bit 9 de `VehicleCallbackMask`) ya se
ejecuta para cada cargo conocido al calcular las opciones de refit. `param1`
lleva la máscara de `CargoClass` del cargo (incluido `CargoSpecDef::classes`)
y `param2` el slot local de la CTT explícita del GRF; sin CTT se aplica el
fallback por versión (`climate_slot` para GRF v1..6 y `bitnum` desde v7).
`CALLBACK_FAILED`/`0` dejan la máscara sin cambios, `1` incluye y `2` excluye;
resultados inválidos son no-op. La decisión catalogue-aware se usa en el
comando de refit, el ciclo de órdenes de depósito, los refits pendientes y
`ReplaceChain`/autoreplace. Las regresiones cubren CTT custom, clases, los
...
Actualización #329-CARGO-CTT-068 (2026-09-04, `9606544b`): los eventos de
`AirportTile` (`CB152` Built/TileLoop/NewCargo/CargoTaken/AcceptanceTick) que
recorren una estación ahora reciben el catálogo `CargoSpec` activo. El
`param2` de `NewCargo`/`CargoTaken` usa el label custom para invertir la CTT del
GRF; construcción y descarga pasan por la variante catálogo-aware y la
...
procesamiento de insumos de la industria, incluso para instancias SAV que aún
no hidrataron sus slots; la API legacy sin catálogo conserva el fallback. La
regresión `TOFU` verifica el `param2` CTT 6. Producción CB1/CB2, callbacks de
tipos dinámicos y aceptación de `IndustryTile` siguen requiriendo una auditoría
catalogue-aware completa; #329 continúa abierto.

Actualización #329-CARGO-CTT-070 (2026-09-04, `391b35d9`): las rutas runtime
de producción industrial CB1/CB2 (`IndustryProductionSpriteGroup`) y de
cargos dinámicos CB14B/CB14C reciben el catálogo `CargoSpec` activo al
resolver índices locales. Labels custom como `TOFU` ya se materializan en
slots, multiplicadores y grupos de producción aunque una instancia provenga de
...
Actualización #329-CARGO-CTT-073 (2026-09-04, `e1f698d3`): los callbacks de
animación de `IndustryTile` (`CB25`/`CB26`/`CB27`) reciben el catálogo
`CargoSpec` activo en `TileLoop`, `IndustryTick`, cambios de construcción,
`CargoReceived`, `CargoDistributed` y el avance de frames visuales. El scope
parent se solicita a partir de las variables realmente usadas por el grafo
Action2, por lo que `0x69`–`0x71` también mantienen la CTT de cargos custom aun
...
custom. El slot global 63 y `CBID_VEHICLE_CUSTOM_REFIT` ya están materializados;
siguen abiertos UI/variables ilimitadas, scopes económicos restantes y otras
propiedades Action0; #329 no se cierra.
...
| Idioma de la UI | **Parcial** | Locale persistente `es`/`en` (también reconoce los identificadores de pack OpenTTD `english*.lng`/`spanish*.lng` para esos dos locales), cambio en vivo y catálogo de etiquetas estáticas de menús, HUD, noticias y ventanas. El HUD dinámico (herramienta, telemetría, alertas operativas, errores de comandos y detalles de estación/depósito), el estado de pausa, el panel de órdenes (título, pool, hint, filas, modos, horarios, refit de carga, incompatibilidades y ruta), la configuración de Noticias (diez categorías, explicación y modos `Off`/`Summary`/`Full`, persistidos por cliente), compra de vehículos (títulos de depósito, ordenar y filtros), selector de paradas viales (título, secciones y prefijo de tipo), selector de estación rail (título, controles y prefijos `Accepts`/`Supplies`), selector de aeropuerto (título, ejes, tamaño y cobertura), selector de destinos (título, hint, botón y fallbacks vanilla), selector de objetos (título, prefijos y objetos vanilla), selector de puentes (título y resumen de tramo), selector de tipo de depósito (título y chips road/rail/ship), pickers de construcción (muelle, boya, waypoints, arbolado, terraformación y cartel), lista de carteles (acciones, estado vacío y encabezado), tooltips de selectores RailType/RoadType (incluida abreviatura eléctrica), los directorios de estaciones, industrias y pueblos (títulos, filtros, estados vacíos, tipos, cadenas y métricas dinámicas), la ficha de industria y su ventana de producción (títulos, estados, tipos vanilla, cadenas, métricas e historial), la ventana de autoridad del pueblo (título, resumen, acciones, estados y ratings), la vista de compañía (título, botón Finanzas y resumen económico), IA / TransCargo, Opciones de visualización (incluido el viewport `TO_*`), CargoDist, configuración NewGRF, Señales PBS y selector de señales, el depósito (título, chrome y edad), la vista de estación (clases, filtros, resúmenes, botones y tooltips), las listas de Órdenes compartidas, Liga y Subvenciones, Autoreemplazo, Sonido y música y el chrome/fallback de Story siguen el locale activo; los títulos de pistas, los reportes técnicos NewGRF, las páginas GameScript, los datos de órdenes, nombres de compañía, cargos, industrias y motores se mantienen literales. Los cuerpos/titulares generados por simulación y los 67 catálogos upstream siguen fuera de alcance (#331) |
...
Actualización #329-INDTILE-RANDOM-034 (2026-09-03, `601e7685`): la
re-randomización `Action2` de `IndustryTile` hidrata y persiste el PSA parent
...
auditoría de la captura naval confirmó un gap de raster que no estaba separado
en el inventario. El modelo conserva `WaterClass` también para
`TileKind::ShipDepot`, y las seis piezas 4070..4075 se posicionan con sus
tamaños/anclas NFO reales (`4070/4071` 64×64, `4072/4073` 32×53 y
`4074/4075` 14×13), mientras los prismas `TILE_SEQ` de `water_land.h` mantienen
los cuatro casos de eje/parte. El ground de Canal ya reproduce la decisión de
`DrawWaterEdges`: agua 4061 más los slots de dique 5380..5391, con recortes y
anclas extraídos del NFO activo y regresiones de selector, spawn y transforms.
...
**LGRP MVP:** load/save del grafo observado (`sav/linkgraph.rs`) ✅ — el
...
NFO y relación con la foundation. La regresión integrada usa un roadtype
...
(`2427 passed; 0 failed; 1 ignored`) y cliente (`1387 passed; 0 failed; 2
...
`cargo_cap` al importar y exportar `VEHS`; el `next` del primario vuelve a
asociar ambos valores sin contaminar la carga principal. Los cuatro caminos de
refit (hangar, estación, autorefit y autoreemplazo) aplican la poda nativa del
correo a la nueva capacidad, evitando reexportar carga imposible después de un
refit a una clase no pasajera. Sigue pendiente materializar la segunda
`VehicleCargoList` y actualizar ese contador durante carga/descarga real.
...
Corrección #326-NEWGRF-OBJECT-ACTION5-FOUNDATIONS (2026-09-13): la ruta
genérica de objetos NewGRF recibe ahora la tabla Action5 de fundaciones activa y
la caché compartida del runtime. Los bloques virtuales de
`FlatteningFoundation` se materializan con su slot, offsets y textura custom
cuando el save los provee; si falta el slot, el fallback queda explícito y
trazable. La regresión ECS usa el bloque 3 (`sprite_id` 5471, slot 58) sobre
...
Corrección #326/#329-AIRPORT-FOUNDATION-SLOPE (2026-09-15, `8bf3c154`): el
renderer aplica `FOUNDATION_LEVELED` antes de dibujar aeropuertos inclinados
tanto en la ruta `MP_STATION/Airport` como en la representación importada
`TileKind::Airport`, igual que `DrawTile_Station`. El suelo de apron y sus
cercas queda como child de la fundación, las capas `BUILD` siguen siendo
parents globales con la altura efectiva y el callback CB150 continúa
controlando los `AirportTile` custom; los docks se mantienen fuera.

`sloped_airports_level_ground_for_station_and_imported_object_paths` verifica
ambos caminos con dos fundaciones, superficie sin césped inclinado, children
con profundidad fuente estable y las capas `StationGfx` sobre la superficie
nivelada. La suite queda en `1541` tests exitosos (2 ignorados), con Clippy
...
Core queda en `2781 passed; 0 failed; 1 ignored`, con Clippy de producción y
...
round-trip y capacidades agregada/local `76/38`. Core queda en `2782 passed; 0
...
direcciones no preferidas. La prueba
`preferred_ship_direction_matches_native_parity_table` cubre los cuatro
trackdirs diagonales y los ocho de esquina; la regresión de coste curvo verifica
que el sesgo se suma al giro correcto. Validación: `2794 passed; 0 failed; 1
ignored` en core, Clippy de producción, check del cliente, formato y
`git diff --check` verdes. #329/#567 siguen abiertos por trackdirs físicos y
cache de ruta completos en el controlador, settings configurables, callbacks y
cachés visuales NewGRF restantes.

Corrección #329/#567-SHIP-PATH-CACHE-TRACKDIR (2026-09-15, `e67fd765`): el
controlador naval consume el `Trackdir` cacheado desde `path.back()`, igual que
`ChooseShipTrack` nativo, y comprueba entrada, orientación y salida contra el
path actual antes de consumirlo. La proyección de teselas se reordena al formato
inverso persistido por OpenTTD; los saltos de acueducto conservan sus dos
entradas y una caché obsoleta se invalida completamente antes del fallback. Las
regresiones `ship_path_cache_uses_native_back_as_next_trackdir`,
`ship_path_cache_invalidates_trackdir_with_wrong_exit` y
`ship_path_cache_preserves_aqueduct_jump_entries` cubren esos contratos.
Validación: `2797` tests de core pasados, `0` fallidos y `1` ignorado, Clippy
...
perfil naval ya puede iniciar YAPF desde el `Trackdir` físico actual, en lugar
de probar cuatro orientaciones rectas potenciales. `Ship::GetVehicleTrackdir`
se replica para depósitos, acueductos, vías ordinarias y unidades accidentadas;
el routing secuencial/paralelo y la selección de múltiples muelles pasan ese
origen al pathfinder. La caché y el coste de ruta separan también esta
orientación, evitando reutilizar una salida calculada para otro rumbo. Las
regresiones cubren la salida permitida/opuesta, la clave de caché y el mapeo de
estado/dirección. Validación: `2802` tests de core pasados, `0` fallidos y `1`
