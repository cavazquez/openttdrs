...
| NewGRF | **Media de parseo / media de runtime** | Catálogos Action0/3/5 amplios; Action5 sigue sus ramas de paisaje/`param[]` durante la carga; call sites reales CB24, CB13/CB14/CB149 y CB140–142 de stations (carga/descarga de cargo, llegada/salida ferroviaria, plataforma ferroviaria, aceptación cada 250 ticks escalonada por StationID, reserva PBS de plataforma y CTT incluidos), y CB13/CB140–142 de RoadStops (construcción/tile loop, carga, llegada/salida vial, aceptación y CTT). RoadStops además parsea `0x0D`, aplica re-randomización Action2 `any`/`all` en NewCargo, CargoTaken, carga, llegada y salida vial, conserva spec/frame/activo/random separados por tesela al crear, serializar y unir RoadStops, y el renderer vuelve a resolver Action3/Action2 con su contexto persistente (vista/tipo/terreno/road/tram/frame/random/triggers/parámetros y los vecinos `66`/`67`/`68`/`6A`/`6B`) con caché por fingerprint de `(var,param)`. Residual: resto CBIDs/scopes/storage; los callbacks/runtime RoadStop aún no tienen vars BaseStation `60`–`65`/`69`, town/company, tipos viales externos, sonidos o mapeo nativo de specs por tesela custom desde `.sav`; Stations también requieren scopes completos y validación diferencial. Ver las [matrices Action0/3/5](parity/newgrf-action0-matrix.md) y de [callbacks](parity/newgrf-callback-matrix.md) |
...
| Idioma de la UI | **Parcial** | Locale persistente `es`/`en`, cambio en vivo y catálogo de etiquetas estáticas de menús, HUD, noticias y ventanas. El HUD dinámico (herramienta, telemetría, alertas operativas y detalles de estación/depósito) y el estado de pausa ya siguen el locale activo. Los cuerpos/titulares generados por simulación y los 67 catálogos upstream siguen fuera de alcance (#331) |
