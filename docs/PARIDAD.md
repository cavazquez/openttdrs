...
| NewGRF | **Media de parseo / media de runtime** | Catálogos Action0/3/5 amplios; Action5 sigue sus ramas de paisaje/`param[]` durante la carga; call sites reales CB24, CB13/CB14/CB149 y CB140–142 de stations (carga/descarga de cargo, llegada/salida ferroviaria, plataforma ferroviaria, aceptación cada 250 ticks escalonada por StationID, reserva PBS de plataforma y CTT incluidos), y CB13/CB140–142 de RoadStops (construcción/tile loop, carga, llegada/salida vial, aceptación y CTT). En vehículos, la longitud efectiva ejecuta CB11 al comprar/refrescar unidades, la carga gradual ejecuta CB12, el refit fija la capacidad efectiva con CB15 y CB10 selecciona el tipo de humo/chispa o desactiva el efecto al renderizar; la propiedad visual Action0 de trenes, carretera y barcos se conserva y sirve como fallback; los callbacks de vehículo hacen writeback de `7C`, y start/stop y sonido siguen conectados a sus call sites. El scope de estación ya expone `0x44` con estado PBS `2/4/7`, `0x45` con continuación de vía de los ocho vecinos, `0x46` centrado, `0x47` centrado con spec homogéneo, `0x49` con filtro de eje y `0x60`–`0x65`/`0x69` con CTT; `0x65` usa el catchment vivo en render, scheduler y triggers cuando reciben el pool de industrias, y sincroniza `HasStationReservation` en `m6`. RoadStops además parsea `0x0D`, aplica re-randomización Action2 `any`/`all` en NewCargo, CargoTaken, carga, llegada y salida vial, conserva spec/frame/activo/random separados por tesela al crear, serializar, unir RoadStops e importar `.sav` (`roadstopspeclist`/`roadstoptiledata`), y el renderer y los CB140–142 vuelven a resolver Action3/Action2 con su contexto persistente (vista/tipo/terreno/road/tram/frame/random/triggers, vars de mundo `45`/`46`/`47`, vars de carga `60`–`65`/`69`, parámetros y los vecinos `66`/`67`/`68`/`6A`/`6B`) con caché por fingerprint de `(var,param)`. La cola de sonidos NewGRF ya llega al mixer Bevy; el callback de sonido de vehículos resuelve salida, marcha, avería, túnel, pago carga/descarga, efecto visual y despegue/aterrizaje con samples globales o locales; los motores vanilla conocidos conservan su muestra específica. Las rutas explícitas reciben tipos viales externos y el picker los agrega al catálogo activo. Los roadtypes conservan las asignaciones Action3 específicas (`ROTSG_BRIDGE`/`ROTSG_OVERLAY`) en una caché separada por selector y el compositor de puentes ya las invoca desde la rampa sur/vano, aplica los offsets de `DrawBridgeRoadBits`, reemplaza la superficie Action5 cuando corresponde y cuelga bridge/overlay del parent trasero combinado. Residual: integrar los grupos de catenaria y la mitad frontal del draw-proc como children, completar layouts/children de estación/objeto/industria/casa y el resto de CBIDs/scopes/storage; el `sfx` custom de Action0 sin callback, CB10 sin geometría completa de offset/potencia de vagones y `CBID_VEHICLE_SPAWN_VISUAL_EFFECT` siguen fuera; las APIs legacy sin catálogo conservan fallback vanilla y los GRF ausentes no pueden reatarse a una vista ejecutable. Ver las [matrices Action0/3/5](parity/newgrf-action0-matrix.md) y de [callbacks](parity/newgrf-callback-matrix.md) |
