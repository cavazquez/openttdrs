...
| NewGRF | **Media de parseo / media de runtime** | Catálogos Action0/3/5 amplios y callbacks reales de estaciones, road stops, casas, industrias, cargos y vehículos (CB10/11/12/13/15/16/24/25–28/31/33/140–149). Vehículos ya resuelven Action2 real por carga, sprite-stack de hasta ocho capas y CB16 con codificación GRF <8/≥8, espejo, ids locales y writeback `7C`; la compra de trenes materializa la cadena articulada, enlaza unidades y recalcula el consist con el catálogo activo. Faltan refresco en reemplazo/movimiento, articulados viales, orientación espejo persistida/renderizada e ids locales extendidos. Los roadtypes resuelven `ROTSG_BRIDGE`/`ROTSG_OVERLAY`/`ROTSG_CATENARY_BACK/FRONT` y los adjuntan a parents combinados; sigue pendiente el fallback vanilla `SPR_TRAMWAY_BASE`, assets completos y layouts/children de estación/objeto/industria/casa, además de scopes/CBIDs restantes, persistencia NGRF/OBJS y `sfx` custom sin callback. Ver las [matrices Action0/3/5](parity/newgrf-action0-matrix.md) y de [callbacks](parity/newgrf-callback-matrix.md) |
...
ordenados por unidad. La compra de trenes materializa las piezas articuladas y
actualiza el consist; articulados viales, refresco en reemplazo/movimiento,
wagon overrides y callbacks que requieren estado completo de consist siguen
siendo residuales.
