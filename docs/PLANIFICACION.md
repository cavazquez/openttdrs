...
**Corte canónico: 2026-08-21 · `main` `cd322d95` · referencia OpenTTD 15.3 `14ec60f`.** Los hitos y fechas anteriores que aparecen más abajo son registro histórico; el estado vigente y los issues abiertos viven en [PARIDAD.md](PARIDAD.md#backlog-sucesor-activo).
...
9. `PLYR` — `CH_TABLE` `name` (string) + `money` (i64) + `colour` (u8) + `is_ai` (u8) + `settings.*` (autorrenovación y servicio por tipo)
...
el runtime; `PLYR` conserva dinero/color/nombre/indicador AI y el subconjunto ejecutado de
`settings.*` por compañía, aunque faltan historial/liveries y flags completos; también faltan `OBJS`,
