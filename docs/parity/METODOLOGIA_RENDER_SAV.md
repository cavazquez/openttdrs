...
| Fundaciones y ascensores de casas | Las casas inclinadas usan el `foundation_draw_plan` común de `DrawFoundation(Leveled)`: el bloque 0–3 se deriva de los vecinos y la superficie de pendientes empinadas conserva su segundo nivel. Cada sprite del plan conserva su `SpriteBounds` C++ (origen, extent y desplazamiento interno), reutilizado por rail, road, estaciones, puentes y paradas. Los parents de edificio vanilla y los sprites directos de `DrawFoundation` pasan por el mismo sorter runtime; el suelo `s1` de la casa y el ascensor de Large Office se registran como children de pantalla del parent que deja activa la fundación, con `SPR_LIFT` en `(14, 60 - posición)`. | Kale completo: las 1.943 fundaciones de casas, sus 1.943 suelos child, 188 ascensores y las 3.014 fundaciones comunes instrumentadas tienen selección, caja y orden relativo contenidos por OpenTTD. El sorter runtime cubre ese subconjunto; faltan las restantes familias, children y clipping de #326. |
