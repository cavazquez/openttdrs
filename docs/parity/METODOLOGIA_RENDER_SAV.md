...
| Fundaciones y ascensores de casas | Las casas inclinadas usan el `foundation_draw_plan` común de `DrawFoundation(Leveled)`: el bloque 0–3 se deriva de los vecinos y la superficie de pendientes empinadas conserva su segundo nivel. El ascensor de Large Office se registra y posiciona como child de pantalla `SPR_LIFT` con `(14, 60 - posición)`. | Kale completo: las 1.943 fundaciones y 188 ascensores se seleccionan y conservan su orden relativo. Los ascensores son children del parent de casa en runtime; las cajas de las fundaciones aún faltan y permanecen en #326. |
...
la caja exacta de `M(...)` y la altura efectiva posterior a la fundación; las
cajas de los 1.943 sprites de fundación siguen fuera del auditor global y son
un residual explícito de #326.
