...
| 3b | [`world-sort`](WORLD_DRAW_SCHEMA.md#orden-global-de-parents-world-sort) | ¿Los parents candidatos se emiten en el orden final de `ViewportSortParentSprites`? | Ya alimenta los slots locales de paradas, depósitos, faroles viales y estaciones rail vanilla (PPP/cable/capas BUILD), más bundles cable/fachada de depósito ferroviario; aún no aplica por sí mismo el sort global, children, pivote ni clipping de las demás familias. |
...
| Fundaciones y ascensores de casas | Las casas inclinadas usan el `foundation_draw_plan` común de `DrawFoundation(Leveled)`: el bloque 0–3 se deriva de los vecinos y la superficie de pendientes empinadas conserva su segundo nivel. Cada sprite del plan conserva ahora su `SpriteBounds` C++ (origen, extent y desplazamiento interno), reutilizado por rail, road, estaciones, puentes y paradas. El ascensor de Large Office se registra y posiciona como child de pantalla `SPR_LIFT` con `(14, 60 - posición)`. | Kale completo: las 1.943 fundaciones de casas, 188 ascensores y las 3.014 fundaciones comunes instrumentadas tienen selección, caja y orden relativo contenidos por OpenTTD. Los ascensores son children del parent de casa en runtime; aplicar el orden global entre esas fundaciones y las demás familias sigue en #326. |
...
vincula 56.088 con el vector final C++. Esto sólo habilita el siguiente paso:
el runtime todavía no aplica el sorter global a esas fundaciones frente a las
familias restantes, residual explícito de #326.
