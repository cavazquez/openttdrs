...
| Catenaria | La ruta común cubre vía normal, cruces a nivel eléctricos, postes de la boca de túnel, cable especial de portal y cable de entrada de depósito. La catenaria road/tram de `MP_ROAD` normal y del road waypoint vanilla sin layout custom publica sus tres recortes traseros y frente como parents con los prismas de `DrawRoadTypeCatenary`; en el waypoint precede a `DrawRailTileSeq(TO_BUILDINGS)` y conserva la altura posterior a foundation. | Kale completo 8bpp: la comparación estricta no deja comandos, geometrías, paletas ni órdenes fuera de OpenTTD; la aceptación raster específica de catenaria vial y la migración de paradas Bus/Truck y waypoints custom siguen pendientes. |
...
waypoints vanilla sin layout custom usan el mismo bloque 4–11 antes de sus
postes BUILD 12/13. En pendiente esos postes y el suelo quedan como children
de la foundation, mientras la catenaria conserva su parent independiente a la
altura efectiva. Las paradas Bus/Truck y los waypoints custom siguen fuera de
ese bloque hasta reconciliar sus ordinales BUILD. El mismo puente incorpora
