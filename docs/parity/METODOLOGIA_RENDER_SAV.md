...
| Catenaria | La ruta común cubre vía normal, cruces a nivel eléctricos, postes de la boca de túnel, cable especial de portal y cable de entrada de depósito. La catenaria road/tram de `MP_ROAD` normal publica sus tres recortes traseros y frente como parents con los prismas de `DrawRoadTypeCatenary`, después de la foundation y antes de roadside. Conserva el orden PPP → PCP antes de las capas `TILE_SEQ` y la altura posterior a fundación. | Kale completo 8bpp: la comparación estricta no deja comandos, geometrías, paletas ni órdenes fuera de OpenTTD; la aceptación raster específica de catenaria vial y la migración de paradas/waypoints siguen pendientes. |
...
waypoints siguen fuera de ese bloque hasta reconciliar sus ordinales BUILD. El
mismo puente incorpora además los edificios
