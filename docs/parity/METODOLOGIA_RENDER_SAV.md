...
| Catenaria | La ruta común cubre vía normal, cruces a nivel eléctricos, postes de la boca de túnel, cable especial de portal y cable de entrada de depósito. La catenaria road/tram de `MP_ROAD` normal, road waypoint vanilla sin layout custom y parada Bus/Truck vanilla o con `TileLayout` NewGRF completo publica sus tres recortes traseros y frente como parents con los prismas de `DrawRoadTypeCatenary`; en estaciones precede a `DrawRailTileSeq(TO_BUILDINGS)` y conserva la altura posterior a foundation. | Kale completo 8bpp: la comparación estricta no deja comandos, geometrías, paletas ni órdenes fuera de OpenTTD; la aceptación raster específica de catenaria vial, los road waypoints con layout NewGRF y los layouts de paradas incompletos siguen pendientes. |
