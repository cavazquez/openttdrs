...
| Fundaciones y ascensores de casas | Las casas inclinadas usan el `foundation_draw_plan` común de `DrawFoundation(Leveled)`: el bloque 0–3 se deriva de los vecinos y la superficie de pendientes empinadas conserva su segundo nivel. El ascensor de Large Office se registra y posiciona como child de pantalla `SPR_LIFT` con `(14, 60 - posición)`. | Kale completo: se recuperaron 1.943 sprites de fundación y 188 ascensores antes ausentes de la traza; los 2.131 draws de casas que faltaban ahora coinciden en selección, fundación, geometría explícita y orden. |
