...
| 2 | RMAP-004: generador procedural | Abierto P1 | Reducir la primera divergencia de TGP/RNG/`FixSlopes`/clear/towns/industries/objects/trees con matriz 64²→512². `GenerateObjects` ya porta transmisor/faro, escala, separación, bytes `MP_OBJECT` y pool `OBJS`; cerrar sólo cuando el mismo seed tenga contrato documentado y sin divergencias no explicadas. |
