...
se cierran #326 o #561. Waypoints y layouts NewGRF permanecen fuera hasta
...
|---|---|---|
| [#326](https://github.com/cavazquez/openttdrs/issues/326) | La composición raster global sigue abierta. `d9b0537c` limita el sorter runtime a la región relevante del viewport y excluye ocultos; el corte incorpora `TileLayoutSpriteGroup` de AirportTile, parents globales para PPP/cables/capas `TILE_SEQ_LINE` rail vanilla con el vidrio de techo como child, las capas BUILD vanilla de paradas Bus/Truck y depósitos viales, y el cable de entrada de túnel eléctrico como parent de su fachada/portal combinados. `VisualCaptureFreeze` además conserva el frame `m7` del SAV antes de la pausa visible, por lo que `2685`/`2678` ya no son falsos positivos de caja aeroportuaria. El culling preciso de PNG usa ahora su rectángulo Bevy real, no el prisma del parent: en el foco Kale reduce faltantes nativos de 29 a 11 y candidatos extra de 116 a 100, pero no equivale a paridad raster. [#560](https://github.com/cavazquez/openttdrs/issues/560) sólo limpia del oráculo los `IncomePopupText` efímeros; [#562](https://github.com/cavazquez/openttdrs/issues/562) calibra la máscara `PALETTE_TO_TRANSPARENT` rail. La evidencia cuantitativa vive sólo en `PARIDAD.md`. Foundations Action5/rotaciones y cajas residuales de aeropuertos, layouts NewGRF/waypoints rail, catenaria/waypoints viales y overlays custom/tranvía de depósitos, sprite-stack y el orden completo de framebuffer siguen sin equivalencia global; el contrato global completo de children queda separado en [#561](https://github.com/cavazquez/openttdrs/issues/561). | Inventariar el siguiente parent nativo sin contraparte tras excluir los frames aeroportuarios animados; priorizar foundations/rotaciones de aeropuerto y repetir seis zooms si se altera viewport, culling u overview. |
