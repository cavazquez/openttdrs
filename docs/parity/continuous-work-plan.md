...
| 2 | RMAP-004: generador procedural | Abierto P1 (TGP y RMAP-005 cerrados; fases posteriores abiertas) | Reducir la primera divergencia de TGP/RNG/`FixSlopes`/clear/towns/industries/objects/trees con matriz 64²→512². La comparación por etapas confirma TGP crudo, ajuste de nivel, costas, suavizados y seno idénticos en 65×65 puntos para la semilla de referencia; RMAP-005 fija fields/desierto/costa y `GetTileZ` de árboles, mientras RMAP-006 alinea la matemática del polígono y su cantidad de grupos. `GenerateObjects` ya porta transmisor/faro, escala, separación, bytes `MP_OBJECT` y pool `OBJS`; el dumper separa las 0x500 pasadas de `RunTileLoop` del tick económico y conserva el RNG global. RMAP-007 sigue con el oráculo pre/post árboles antes de atribuir diferencias al algoritmo. Cerrar sólo cuando el mismo seed no tenga divergencias no explicadas en clear, población, objetos, árboles y bytes de tesela. |
