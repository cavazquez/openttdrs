...
| 2 | RMAP-004: generador procedural | Abierto P1 (TGP cerrado, fases posteriores abiertas) | Reducir la primera divergencia de TGP/RNG/`FixSlopes`/clear/towns/industries/objects/trees con matriz 64²→512². La comparación por etapas confirma TGP crudo, ajuste de nivel, costas, suavizados y seno idénticos en 65×65 puntos para la semilla de referencia; el límite fraccionario de costa queda cubierto por regresión. `GenerateObjects` ya porta transmisor/faro, escala, separación, bytes `MP_OBJECT` y pool `OBJS`; el dumper separa ahora las 0x500 pasadas de `RunTileLoop` del tick económico y conserva el RNG global de generación. Cerrar sólo cuando el mismo seed tenga contrato documentado y sin divergencias no explicadas en clear, población, objetos, árboles y bytes de tesela. |
