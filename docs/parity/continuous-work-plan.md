...
| 3 | Composición raster global (#323→#322→#326) | En curso | El sorter runtime ya cubre piezas estructurales, catenaria, el bloque combinado PBS/Action5/tranvía de puentes y cuerpos/unidades de vehículos con cajas `M(...)`, children y orden de inserción estable; el overlay vanilla de tranvía de un puente medio resuelve la rampa sur y los offsets específicos `0,1,11..14` de `DrawBridgeRoadBits`. Los overlays NewGRF de carretera y estación rail (incluidas pendientes niveladas) siguen la fundación cuando existe. Siguen pendientes mitad frontal de puente y layouts/children completos de estación/objeto/industria/casa; el sprite-stack de vehículos ya materializa hasta ocho capas runtime con Action2 real y var `0x10`, pero conserva límites de callbacks/paleta. Las capturas 4×4 siguen siendo diagnóstico, no único oracle. |
...
| 5 | NewGRF runtime (#329) | Abierto | Vehículos, estaciones, objetos e industrias ya tienen rutas runtime parciales; vehículos además resuelven grupos Action2 real por etapa cargada/cargando y hasta ocho capas de sprite-stack, y casas reevalúan Action2 por tesela (etapa/hash, edad, terreno, frame, posición y random/triggers). Los roadtypes conservan grupos Action3 específicos por selector y la caché los separa por `ROTSG_*`; aún falta conectarlos al draw-proc de puentes y completar la composición frontal. Siguen pendientes callbacks/layouts completos de casas, vehículos, estaciones, aeropuertos, objetos, industrias y cargos, además de persistencia NGRF/OBJS. |
...
- Composición #326: el bloque publicado de puentes enlaza cabezas de rampa,
  ...
  de agua intermedia. Sigue pendiente la mitad frontal de
