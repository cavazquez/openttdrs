...
| 2 | RMAP-004: generador procedural | Abierto P1 (TGP y RMAP-005/RMAP-008 cerrados; fases posteriores abiertas) | Reducir la primera divergencia de TGP/RNG/`FixSlopes`/clear/towns/industries/objects/trees con matriz 64²→512². La comparación por etapas confirma TGP crudo, ajuste de nivel, costas, suavizados y seno idénticos en 65×65 puntos para la semilla de referencia; RMAP-005 fija fields/desierto/costa y `GetTileZ` de árboles, mientras RMAP-006 alinea la matemática del polígono y su cantidad de grupos. `GenerateObjects` ya porta transmisor/faro, escala, separación, bytes `MP_OBJECT` y pool `OBJS`; el dumper separa las 0x500 pasadas de `RunTileLoop` del tick económico y conserva el RNG global. RMAP-008 preserva ese RNG en `DATE` al importar y escribir SAV, y RMAP-007 ya puede replayar árboles sobre el preestado real; ahora debe versionar el oráculo pre/post y reducir su primera colocación divergente. Cerrar sólo cuando el mismo seed no tenga divergencias no explicadas en clear, población, objetos, árboles y bytes de tesela. |
