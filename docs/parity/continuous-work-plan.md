...
de última aceptación de cada slot. PSA de industria y aeropuerto ya tiene importación,
...
#329 y el bloque SAV siguen abiertos por callbacks de tesela, storages de
pueblos/casas/objetos, cargos custom y callbacks restantes.
...
decodifica `INDY.psa`, `STNN.normal.airport.psa` y `PSAC`, hidrata los registros
no nulos en `Industry` o `Station` y el exportador vuelve a emitir el pool
completo con índices densos y 256 valores por fila, sin descartar storages de
pueblos/casas/objetos que aún no tienen runtime. Quedan pendientes el
