...
|---|---|---|
| [#326](https://github.com/cavazquez/openttdrs/issues/326) | La composición raster global sigue abierta. `d9b0537c` limita el sorter runtime a la región relevante del viewport y excluye ocultos; el corte incorpora `TileLayoutSpriteGroup` de AirportTile, parents globales para PPP/cables/capas `TILE_SEQ_LINE` rail vanilla con el vidrio de techo como child, las capas BUILD vanilla de paradas Bus/Truck y depósitos viales, y el cable de entrada de túnel eléctrico como parent de su fachada/portal combinados. [#560](https://github.com/cavazquez/openttdrs/issues/560) sólo limpia del oráculo los `IncomePopupText` efímeros; [#562](https://github.com/cavazquez/openttdrs/issues/562) calibra la máscara `PALETTE_TO_TRANSPARENT` rail. La evidencia cuantitativa vive sólo en `PARIDAD.md`. Foundations Action5/rotaciones y cajas residuales de aeropuertos, layouts NewGRF/waypoints rail, catenaria/waypoints viales y overlays custom/tranvía de depósitos, sprite-stack y el orden completo de framebuffer siguen sin equivalencia global; el contrato global completo de children queda separado en [#561](https://github.com/cavazquez/openttdrs/issues/561). | Ajustar o clasificar las dos capas aeroportuarias visibles que aún no igualan su caja nativa; después verificar `0,12×`, `0,25×`, `0,50×`, `1×` y los seis zooms si cambia viewport, culling u overview. |
