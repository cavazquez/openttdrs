...
| 3 | Composición raster global (#323→#322→#326) | En curso | El sorter runtime ya cubre piezas estructurales, catenaria, PBS/Action5/tranvía de puentes y cuerpos/unidades de vehículos con cajas `M(...)`, children y orden estable. Paradas, waypoints viales, estaciones rail y objetos resuelven layouts `TileSeq` completos por Action3/2→Action1, materializan suelo, parents/children y pendientes; el procesador aplica `DODRAW`, offsets de sprite/caja/child, `var10`, draw mode `0x100` e invalida la caché con registros `7D`/`0x100`. Sprites base y paletas custom siguen fallback atómico. Continúan pendientes los layouts/children completos de industrias y casas, las variantes ferroviarias de pendiente/túnel/depósito y el sprite-stack/callbacks avanzados de vehículos. Las capturas 4×4 siguen siendo diagnóstico, no único oracle. |
