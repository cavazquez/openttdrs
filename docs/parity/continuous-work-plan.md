...
| 2 | RMAP-004: generador procedural | Abierto P1 (TGP, RMAP-005/RMAP-007/RMAP-008/RMAP-009 cerrados; fases posteriores abiertas) | Reducir la primera divergencia de TGP/RNG/`FixSlopes`/clear/towns/industries/objects/trees con matriz 64²→512². La comparación por etapas confirma TGP crudo, ajuste de nivel, costas, suavizados y seno idénticos en 65×65 puntos para la semilla de referencia; RMAP-005 fija fields/desierto/costa y `GetTileZ` de árboles. El oráculo pre/post versionado de RMAP-007 retoma el RNG de `DATE` y verifica el fixture temperate 64² por tesela, bloques 4×4 y `PlaceTree`; RMAP-009 además fija el escalado `GetTileZ()*2*15/map_height_limit`. Sigue el diferencial ártico/tropical y la primera divergencia de las fases anteriores. Cerrar sólo cuando el mismo seed no tenga divergencias no explicadas en clear, población, objetos, árboles y bytes de tesela. |
