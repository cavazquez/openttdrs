...
|---|---|---|
| [#326](https://github.com/cavazquez/openttdrs/issues/326) | La composición raster global sigue abierta. El sorter incorpora `TileLayoutSpriteGroup` de AirportTile; parents globales para PPP/cables/capas `TILE_SEQ_LINE` rail vanilla, paradas Bus/Truck, depósitos viales, cable de entrada de túnel eléctrico, los cuatro postes vanilla de road waypoint en plano y catenaria road/tram de calles normales, road waypoints vanilla y paradas Bus/Truck vanilla sin layout custom. El vidrio rail y los postes en pendiente conservan su relación child con el parent correspondiente. `VisualCaptureFreeze` evita falsos positivos animados y el culling usa el rectángulo real de Bevy, pero ninguno equivale a paridad raster. Kale no contiene un foco vial de waypoint/catenaria reproducido, así que esas migraciones sólo tienen evidencia estructural. Foundations/rotaciones aeroportuarias, layouts custom/NewGRF, waypoints rail, catenaria de paradas/waypoints con layout custom, overlays custom/tranvía de depósitos, sprite-stack, clipping, pivotes y framebuffer siguen sin equivalencia global; el contrato global completo de children queda separado en [#561](https://github.com/cavazquez/openttdrs/issues/561). | Inventariar el siguiente parent nativo sin contraparte tras excluir los frames aeroportuarios animados; priorizar foundations/rotaciones de aeropuerto y repetir seis zooms si se altera viewport, culling u overview. |
