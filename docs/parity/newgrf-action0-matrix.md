...
| `21` extra miscellaneous flags | **runtime parcial** (preserva el `DWord` en `EngineDef.extra_flags`; `SyncReliability` sigue `variant_parent_id` al materializar, autoreemplazar y prestar servicio a vehículos; `NoNews`, `NoPreview` y `JoinPreview` pendientes) |
...
| `09` sprite layouts | **runtime parcial**: lee `ExtendedByte`, ground y secuencias BUILD/child clásicas con cajas de parents, interpreta la inversión histórica de Action1 y las materializa por Action1/3; normaliza los modifiers nativos y la caché sólo aplica la paleta por defecto cuando `transparent`/`recolour` lo declaran. Las tablas vanilla internas, la composición destino transparente y paletas no representables conservan fallback |
| `1A` advanced sprite layouts | **runtime parcial**: conserva orientación X/Y, origen/extensión y registros `DODRAW`/sprite/paleta/`var10`; valida ground, flags desconocidos y límites nativos antes de materializar, permitiendo `var10` sólo en la cadena de Stations autorizada. Normaliza los bits nativos `opaque`/`transparent`/`recolour` fuera de los IDs y los conserva en el resultado resuelto; la caché aplica la paleta por defecto sólo cuando corresponde, pero el blitter cliente todavía no compone `transparent` ni hace prevalecer `opaque` sobre ocultar. Action2 mantiene `allow_var10=false`; paletas especiales, relocación y scopes restantes siguen pendientes |
...
Labels custom o de climas no representados por `CargoType` siguen en fallback.
