...
| `18` animation triggers | **runtime parcial**: máscara CB140 para `Built`, `TileLoop`, `NewCargo`, `CargoTaken`, `VehicleLoads` y `VehicleArrives` de tren (`TA_PLATFORM` al entrar en `BeginLoading`), `AcceptanceTick` (`TA_WHOLE`, cada 250 ticks escalonado por StationID) y `PathReservation` (`TA_PLATFORM` al reservar por primera vez una tesela de estación). El disparador entrega el ordinal correcto en el byte bajo de `param2`; para carga, el byte alto usa la CTT Action0 GlobalVar `0x09` (o el bitnum/clima según versión GRF). Faltan `VehicleDeparts`, road stops y sonidos. |
