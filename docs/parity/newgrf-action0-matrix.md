...
| `1A` advanced sprite layouts | **runtime parcial**: conserva orientación X/Y, origen/extensión y registros `DODRAW`/sprite/paleta/`var10`; valida ground, flags desconocidos y límites nativos antes de materializar, permitiendo `var10` sólo en la cadena de Stations autorizada. Action2 mantiene `allow_var10=false`; paletas especiales, transparencia, relocación y scopes restantes siguen pendientes |
