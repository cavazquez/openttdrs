...
| Vehicles (`00`–`03`) | `0x15` `CBID_VEHICLE_REFIT_CAPACITY` | **parcial runtime** | `resolve_vehicle_current_refit_capacity` aplica CB15 sólo cuando el cargo actual difiere del cargo Action0 o hay un subtipo activo, y trata su resultado como capacidad final, sin volver a aplicar el multiplicador de CargoSpec. La decisión tiene prioridad sobre CB36 en `ConsistChanged`, autoreemplazo, piezas articuladas, refit manual y auto-refit de estación; el refresco recalcula también la suma visible de la cabeza del consist. `CALLBACK_FAILED` cae a Action0/CB36 y conserva cero como resultado válido. Falta modelar `NoDefaultCargoMultiplier`, la selección completa de subtipos, la capacidad secundaria de aeronaves, el balanceo completo de consist y las APIs legacy sin catálogo |
