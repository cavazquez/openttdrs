...
| Stations | `0x149` `CBID_STATION_LAND_SLOPE_CHECK` | **parcial runtime** | Bit `SlopeCheck` de Action0 `0x0B`; Action3→Action2 por tesela en query+execute de `PlaceRailStation` / `PlaceRailStationArea`, antes de mutar. `param1` conserva slope+orientación; `param2` andenes/longitud/offsets. `FAILED`/`0x400` permite en GRF ≥8 y se aplica la inversión de bit 10 para GRF <8. Los resultados de rechazo conservan diagnóstico efímero: `0..0x3FF`/`0x40F` llegan al HUD como texto Action4/Action13 y los códigos estándar/desconocidos mantienen fallback genérico. Faltan scope de estación/vecinos completos y parámetros dinámicos del text stack. |
