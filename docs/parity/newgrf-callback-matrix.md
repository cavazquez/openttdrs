...
| Vehicle/station random Action2 (`0x80`/`0x83`/`0x84`) en resolve de sprites | **Parcial** (eval con `random_bits` de 16 bits; reseed gameplay vía `trigger_vehicle_randomisation_chain`; vehículos ya consumen `Callback32` y conservan triggers pendientes) |
...
13. Vehicles CB32 — barrido económico escalonado, `day_counter` persistente, palabra de randomización de 16 bits, bits de paleta y trigger `Callback32` con reseed del grupo Action2 activo; `trigger_vehicle_randomisation_chain` cubre la propagación de `NewCargo`/`AnyNewCargo`, `Depot` y `Empty`. Pendientes los call sites de eventos y la invalidación de paleta del renderer.
