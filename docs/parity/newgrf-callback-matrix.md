...
| Industries (`0A`) | `0x28` `CBID_INDUSTRY_LOCATION` | **parcial runtime** (#266) | Call site: `place_industry_spec_def_sandbox`; respeta el bit `Location`, carga Action3→Action2, pasa `IACT_USERCREATION` (`param2=2`) y expone el scope básico de construcción (`0x80`/`0x81` TileIndex, `0x82` pueblo, `0x86` layout, `0x87` terreno, `0x88` zona, `0x89`/`0x8D` distancia, `0x8A` altura), permitiendo sólo `FAILED`/`0x400`. Siguen pendientes badges `0x7A`, distancia al agua `0x8B`, random/flags completos, otros tipos de creación y strings de error GRF |
...
| Industries | Scope `0x40`–`0xB4` y callbacks restantes | **parcial runtime** | El scope padre de las teselas expone esperas, distancia al agua, layout/posición/dimensiones, cargos/salida, producción mensual, nivel, color aleatorio, contador y random; los datos ausentes del modelo nativo (fundador, fechas, flags, PSA y cargos custom) mantienen cero/fallback explícito. `IndustrySpecDef.callback_mask` conserva callbacks aún no conectados. |
