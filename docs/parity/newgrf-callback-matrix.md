...
| Houses (`07`) | Action0 `0x1A`/`0x1B`; CB1A/CB1B/CB1C | **metadata + ANIT preservado** | El catálogo conserva `animation_frames`, estado no-loop/loop (`0xFF` si falta la propiedad) y velocidad limitada a `2..=16`, además de las máscaras nombradas de next-frame, TileLoop y velocidad. La subsecuencia urbana de `ANIT` se persiste/importa en una cola común, conserva el orden relativo lift/NewGRF y acepta el alias JSON histórico; hasta conectar CB1A una entrada NewGRF activa retiene su slot sin consumir RNG. Scheduler y call sites de esos callbacks siguen pendientes. |
