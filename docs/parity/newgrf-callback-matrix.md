...
Actualizada: **2026-09-03** (commit `9e01c1a9`, shape-check y pendientes de
teselas de industria; triggers y PSA parent ya publicados).
...
| Industry tiles (`09`) | `0x3C` `CBID_INDTILE_AUTOSLOPE` | **almacenado** | `IndustryTileSpecDef` conserva la máscara `IndustryTileCallbackMask::Autoslope` y el identificador upstream; todavía no se ejecuta al terraformar una tesela de industria. El comando de terraformación actual mantiene bloqueadas las teselas `Industry`, por lo que falta un call site seguro con la pendiente prospectiva. |
| Industry tiles | Action2 vars `0x40`–`0x44`, `0x60`–`0x62`, `0x7A` | **parcial runtime** | El renderer, shape-check y los callbacks de animación construyen el scope por tesela con `m3` como random (incluidos triggers), etapa de obra, terreno, zona del pueblo más cercano, posición relativa, frame completo `m3hi`, información/frame/id de teselas vecinas y presencia de badges mediante GlobalVar `0x18`; el scope parent comparte stock/producción/historial de la `Industry` y sus registros PSA. El mismo contexto alimenta vistas planas y layouts `TileSeq`, con caché por fingerprint. `0x62` conserva los sentinelas `0xFFFF`/`0xFFFE` y traduce el local del mismo GRF. Faltan el resto de variables específicas del tile y callbacks de sonido/autoslope. |
