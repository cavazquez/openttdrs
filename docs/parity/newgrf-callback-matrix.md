...
| Industries (`0A`) | `0x28` `CBID_INDUSTRY_LOCATION` | **soportado** (#266) | Call site: `place_industry_spec_def_sandbox`; respeta el bit `Location`, carga Action3→Action2 y aplica la inversión histórica del bit 10 para GRF <8 (`FAILED` conserva fallback; `0x400` es éxito desde GRF 8). Faltan scopes/parametrización completos y strings de error GRF |
