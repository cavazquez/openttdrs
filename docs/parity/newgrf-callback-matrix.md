...
| RoadStops (`14`) | Action2 parent `TownScopeResolver` | **parcial runtime** | La ruta map-aware selecciona el pueblo más cercano con la misma distancia Manhattan/ID determinista que `0x45`/`0x46`, copia las variables de `TownScopeResolver` modeladas y carga el PSA `7C` por GRFID; las APIs legacy sin pool de pueblos conservan el parent vacío. La asociación nativa parada→pueblo y los campos/variables todavía no representados siguen pendientes. |
