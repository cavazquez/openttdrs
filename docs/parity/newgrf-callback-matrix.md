...
| RoadStops (`14`) | Action0 props `0x13`/`0x14` | **parcial runtime** | El parser consume el `ExtendedByte` y conserva hasta seis layouts en `RoadStopSpecDef.bridgeable_info`: `0x13` fija la altura mínima y `0x14` la máscara de pilares no permitidos, con defaults cero idénticos a `RoadStopSpec` de OpenTTD. Los bytes que exceden los seis layouts se consumen para mantener alineada la siguiente propiedad (`0x15`/`0x16`), y el catálogo/aplicación y JSON los preservan. `PlaceRoadBridge` ya llama al chequeo por tesela y rechaza `min_height=0` o un tablero por debajo de `GetTileMaxZ + min_height`; las máscaras de pilares aún sólo están almacenadas y su compositor visual sigue pendiente. |
...
preview (`command_would_fail`). `disallowed_pillars` sigue conservándose para
el dibujo de pilares y queda pendiente un corte de render separado.
