...
| Houses (`07`) | Action0 `0x1A`/`0x1B`; CB1A/CB1B/CB1C | **metadata** | El catálogo conserva `animation_frames`, estado no-loop/loop (`0xFF` si falta la propiedad) y velocidad limitada a `2..=16`, además de las máscaras nombradas de next-frame, TileLoop y velocidad. El scheduler, la cola `ANIT` compartida y los call sites de esos callbacks siguen pendientes. |
