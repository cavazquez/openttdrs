...
- Resto de CBs houses / airports / industries / objects (excepto CB157), cargo (excepto CB39/CB145). Stations aún requieren scopes completos y sonidos; RoadStops ya resuelve `45`/`46`/`47`, `60`–`65`/`69` y `66`/`67`/`68`/`6A`/`6B` al renderizar, en CB140–142 y en la randomización. La importación `.sav` conserva el mapeo nativo `(GRFID, localidx)` y el estado de cada tesela; quedan tipos externos en picker/API legacy, sonidos y el caso inevitable de GRF no instalado.
