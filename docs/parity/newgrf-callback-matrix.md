...
Actualizada: **2026-09-03** (commit `389109c1`, shape-check, foundations,
autoslope, color, rechazo temporal y cargos dinámicos de industrias; triggers
y PSA parent ya publicados).
...
| Industries | `0x14B` `CBID_INDUSTRY_INPUT_CARGO_TYPES`; `0x14C` `CBID_INDUSTRY_OUTPUT_CARGO_TYPES` | **parcial runtime** | Al fundar una industria con runtime Action2, los callbacks se consultan por slot (`param1=0..2`, `param2=0`) y reemplazan las listas estáticas de entradas/salidas; `CALLBACK_FAILED`, `0xFF`, valores fuera de 8 bits, cargos no declarados o duplicados terminan la secuencia. El índice local se valida contra la tabla CTT del GRF, los multiplicadores `0x1C`–`0x1E` conservan el índice estático de origen y una lista de salida vacía no vuelve al cargo vanilla. Sin runtime se mantiene el fallback estático. El límite actual es el esquema histórico de 3 entradas/2 salidas: `CargoTypesUnlimited`, cargos custom no representables y rehidratación de estas listas desde SAV siguen pendientes. |
...
| Industries | `0x22`, `0x37`–`0x3B`, `0x14D`+, … | **almacenado** | `IndustrySpecDef.callback_mask` |
