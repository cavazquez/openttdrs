...
| RoadStops (`14`) | Action0 props `0x13`/`0x14` | **almacenado/runtime parcial** | El parser consume el `ExtendedByte` y conserva hasta seis layouts en `RoadStopSpecDef.bridgeable_info`: `0x13` fija la altura mínima y `0x14` la máscara de pilares no permitidos, con defaults cero idénticos a `RoadStopSpec` de OpenTTD. Los bytes que exceden los seis layouts se consumen para mantener alineada la siguiente propiedad (`0x15`/`0x16`), y el catálogo/aplicación y JSON los preservan. La comprobación de altura/pilares al construir un puente sobre una parada todavía no está conectada al comando de puentes. |
...
comprobación de altura/pilares al intentar tender un puente sobre una parada
todavía no está conectada al comando de puentes; queda como el siguiente
consumidor de este dato dentro de #329.
