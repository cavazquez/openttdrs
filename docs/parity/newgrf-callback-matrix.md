...
Actualizada: **2026-09-04** (commit `1d20248f`, CB160 de efectos visuales
...
| Vehicles (`00`–`03`) | `0x10` `CBID_VEHICLE_VISUAL_EFFECT` | **parcial runtime** | `resolve_vehicle_visual_effect_callback` ejecuta el callback con `param1=param2=0`, normaliza bits de tipo (vapor/diésel/chispa) y `VE_DISABLE_EFFECT`, conserva offset/bit de potencia en `VehicleVisualEffectSpec` y escribe `7C`. La propiedad Action0 (`train 0x22`, `road 0x21`, `ship 0x1C`) se conserva en el catálogo y actúa como fallback cuando el callback falla. El emisor de humo ferroviario usa el catálogo activo y respeta el resultado; road/ship/air y la semántica completa de offset/potencia de vagones siguen parciales. |
| Vehicles (`00`–`03`) | `0x160` `CBID_VEHICLE_SPAWN_VISUAL_EFFECT` | **parcial runtime** | Cuando CB10 selecciona un modelo avanzado, `resolve_vehicle_spawn_visual_effect_callback` ejecuta Action2 con RNG, hace writeback de `7C`, decodifica cantidad, `auto_center`, `auto_rotate` y los cuatro registros `0x100..0x103` como tipo + offsets signed X/Y/Z. El renderer de trenes materializa los tipos vanilla `F1`/`F2`/`F3`/`FA` como efectos separados y rota X/Y según la dirección; un modelo reservado/desactivado o `CALLBACK_FAILED` no cae a humo vanilla. Faltan call sites de road vehicles, barcos y aeronaves, sprites/sonidos locales de GRF y la proyección exacta de `auto_center`/offsets en todas las escalas. |
...
disponible para la siguiente evaluación. El renderer de trenes dibuja los
efectos vanilla `F1`/`F2`/`F3`/`FA` y rota los offsets según dirección. Los
...
road/ship/air, sprites y sonidos locales, y la semántica exacta de
`auto_center`/offsets y scopes completos.
