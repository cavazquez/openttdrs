...
Actualizada: **2026-09-03** (commit `628d1fb9`, shape-check, foundations,
autoslope, color, rechazo temporal, cargos dinámicos, efectos especiales y
`PlantOnBuild` de industrias; triggers, PSA parent y `CargoTypesUnlimited` ya
publicados).
...
| Industries | `0x3D` `CBID_INDUSTRY_REFUSE_CARGO` | **parcial runtime** | El callback se ejecuta para cada entrada de una procesadora NewGRF después de comprobar que el lote requerido está disponible y antes de retirarlo de las estaciones. `param1=0` y `param2` recibe el índice de cargo local traducido por el label CTT; un resultado no nulo acepta y cero rechaza (OpenTTD invierte `ConvertBooleanCallback`); `CALLBACK_FAILED`, runtime ausente o cargo no traducible conserva la aceptación. Una negativa deja intacto el stock de estación y no dispara CB1/CB2 ni `CargoReceived`. El modelo aún no materializa `DeliverGoodsToIndustry` como entidad separada, por lo que la consulta está acoplada al ciclo de procesamiento y faltan scopes/temporización de entrega directa. |
| Industries | `0x14B` `CBID_INDUSTRY_INPUT_CARGO_TYPES`; `0x14C` `CBID_INDUSTRY_OUTPUT_CARGO_TYPES` | **parcial runtime** | Al fundar una industria con runtime Action2, los callbacks se consultan por slot (`param1`, `param2=0`) y reemplazan las listas estáticas de entradas/salidas; `CALLBACK_FAILED`, `0xFF`, valores fuera de 8 bits, cargos no declarados o duplicados terminan la secuencia. El índice local se valida contra la tabla CTT del GRF y los multiplicadores `0x1C`–`0x1E` conservan el índice estático de origen. `prop 0x1A` ya se parsea como `IndustryBehaviour`: con `CargoTypesUnlimited` se recorren hasta 16 entradas/salidas y las salidas desde el tercer slot se conservan en `newgrf_extra_output_cargos`/`newgrf_extra_produced_cargo` para transporte y SAV. Sin runtime se mantiene el fallback estático. La economía procesa tasas, multiplicadores, stocks y transporte de todas las salidas declaradas; siguen pendientes slots vacíos legacy, cargos custom y rehidratación runtime de estas listas desde SAV. |
| Industries | `0x3B` `CBID_INDUSTRY_SPECIAL_EFFECT` | **parcial runtime** | El ciclo nativo de 256 ticks evalúa `PlantFields` (`param2=0`) y `CutTrees` (`param2=1`) con `Random()` en `param1`, escribe de vuelta `7C` y convierte el resultado booleano; `CALLBACK_FAILED` conserva el fallback OpenTTD. PlantFields reutiliza la geometría/RNG de `PlantFarmField` y CutTrees recorre la espiral 40×40, exige la huella terminada, corta el primer árbol adulto y añade 45 unidades al primer output. Las industrias vanilla Farm/FarmTropic/LumberMill respetan los flags equivalentes. `PlantOnBuild` ya ejecuta 50 intentos en la colocación manual vanilla y NewGRF; faltan afterload/rehidratación SAV, escalas/sonidos y goldens tick-a-tick. |
...
| Persistente industria | `Industry.newgrf_persistent_regs` | Writeback tras CB de producción, CB25/26/27 y `ResolveRerandomisation` de tesela para `TileLoop`, `IndustryTick` y `CargoReceived` cuando el scheduler recibe pools de mundo; reseed parent agregado una vez por footprint; round-trip SAV/JSON, con `INDY.psa`/`PSAC` conservados |
...
24. Industries CB3B — efectos especiales periódicos de campos/árboles y `PlantOnBuild` manual/NewGRF; call sites y fallback implementados, afterload SAV/goldens aún pendientes.
