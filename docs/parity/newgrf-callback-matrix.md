...
| Vehicles (`00`–`03`) | Action0 flags de motor (`0x27`/`0x1C`/`0x17` + `0x30`/`0x27`/`0x21`) | **parcial runtime** | El parser y el catálogo conservan las máscaras nativas para trenes, carretera, barcos y aeronaves. `NoDefaultCargoMultiplier` alimenta CB15; `NoBreakdownSmoke` suprime sólo el FX de humo de avería en el puente de eventos del cliente, manteniendo el SFX. `SyncReliability` resuelve la cadena de variantes. `NoNews`, `NoPreview` y `JoinPreview` quedan preservados sin efecto hasta modelar el ciclo de disponibilidad/previews. |
