...
- Storage persistente de industria/aeropuerto: `INDY.psa`, `STNN.normal.airport.psa` y `PSAC` se importan, hidratan y exportan para los registros `7C`; siguen pendientes el writeback de teselas, la invalidación tras mutaciones y los storages/callbacks de casa, pueblo y objeto. CB140–142 preserva `7C` de la estación pero no los scopes/áreas completos de `BaseStation`; CB14 aún no aporta el scope/regs de `BaseStation` ni layout 16-bit exacto; CB149 aún no aporta scope/vecinos ni strings GRF.
