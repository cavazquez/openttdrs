...
| Concepto OpenTTD | Referencia C++ | Equivalente Rust | Validación |
|---|---|---|---|
| Bahía (bay stop) bus/camión — el vehículo ENTRA a la tesela | `roadveh_cmd.cpp:1311-1330`, tablas `_rv_station_*` (`roadveh_movement.h:1052-1083`) | Fase 2: destino = tesela de la bahía (`station.rs::resolve_order_destination` + `is_connected_bay_road_stop`); la carga ocurre dentro. Trayectoria interna aproximada (carril recto), sin las 32 tablas `_rv_station_*` | chequeo `bay_stop_position` como regresión; tests de `station.rs` y `parity_system.rs` |
| Frame exacto de parada en bahía | `_road_stop_stop_frame` (`roadveh_movement.h:1087-1093`, valores 11–20) + chequeo `roadveh_cmd.cpp:1496-1502` | Aproximado en render: `road_movement.rs::BAY_STOP_PROGRESS = 128` (centro del carril de entrada) | golden verifica rango 11–20 del fixture; punto exacto por boca/lado pendiente |
