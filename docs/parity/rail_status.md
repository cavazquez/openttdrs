...
| Railtypes / electrificación / conversión | `rail_type.rs` + `ConvertRail` + catenaria plana | `rail.h`, `elrail.cpp` / `elrail_data.h` | 2 · probado (Fase 5–6 + wires) | `convert_rail_*`, `collect_catenary_flat_*`, `*_engine_requires_*` | Medio: catenaria plana MVP; sin PCP/pendientes/túneles; tranvía = RoadType |
