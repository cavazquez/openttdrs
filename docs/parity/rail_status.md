...
| Railtypes / electrificación / conversión | `rail_type.rs` + `ConvertRail` + catenaria | `rail.h`, `elrail.cpp` / `elrail_data.h` | 2 · probado (Fase 5–6 + wires) | `convert_rail_*`, `collect_catenary_*`, `*_engine_requires_*` | Medio: catenaria plano+pendiente X/Y; sin PCP vecinos/túneles; tranvía = RoadType |
