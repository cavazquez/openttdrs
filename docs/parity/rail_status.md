...
| Reservas de camino (PBS) | `rail_pbs.rs` (TryReserve, `follow_train_reservation`, path signals, plataforma) | `pbs.cpp` (`TryReserveRailTrack`, `FollowTrainReservation`) | 2 · probado (+ golden interno) | `golden_pbs.rs`, `follow_train_reservation_*`, `train_pbs_*`, fixture/oráculo externo JSONL | Medio: initial coincide; primer tick diverge en cinemática (Rust progreso 115/velocidad 64; OpenTTD 159/73) |
...
| Paso sub-tesela (`progress` 0–255) | compartido con carretera (`progress_step_for_speed`, 192/256; paso ref. 112) | `vehicle_base.h:439-454` | 3 · validado (heredado del golden de carretera) | `tests/golden_roadveh.rs` | Bajo–medio (tick ~37 Hz) |
...
| Oráculo PBS externo | Fixture `train_pbs_15_3.sav` + 40 ticks OpenTTD versionados — [PBS_EXTERNAL_ORACLE.md](../PBS_EXTERNAL_ORACLE.md); primera divergencia: velocidad/estado de movimiento `VEHS` no importado |
