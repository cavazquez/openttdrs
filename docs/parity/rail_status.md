...
| Railtypes / electrificación / conversión | `rail_type.rs` + `ConvertRail` (Rail/Electric/Mono/Maglev) | `rail.h:26-525` (`RailTypeInfo`), `CmdConvertRail` | 2 · probado (Fase 5–6 MVP) | `convert_rail_*`, `*_engine_requires_*`, `monorail_path_does_not_cross_*` | Medio: sprites planos mono/maglev; sin catenaria ni pendientes tipadas; tranvía = RoadType |
