...
| Railtypes / electrificación / conversión | `rail_type.rs` + `ConvertRail` (normal/eléctrico) | `rail.h:26-525` (`RailTypeInfo`), `CmdConvertRail` | 2 · probado (Fase 5 MVP) | `convert_rail_*`, `electric_engine_requires_*` | Medio: sin catenaria visual; mono/maglev → Fase 6 |
