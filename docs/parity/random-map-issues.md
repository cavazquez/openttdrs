...
| **RMAP-007** | Oráculo de frontera pre/post `GenerateTrees`. | **Abierto (P1)** | Exportar desde OpenTTD el mapa y estado RNG inmediatamente antes y después de `GenerateTrees`, y reproducirlos en Rust sobre la misma entrada. Cerrar sólo con fixtures que localicen la primera diferencia de árbol sin confundirla con clear, pueblos, industrias u objetos. |
