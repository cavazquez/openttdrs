...
| **RMAP-006** | Diferencial completo de la fase de árboles. | **Abierto (P1, avance parcial)** | Ya se alinearon la suma `GB(Random(), 0, 5) + 25`, el divisor de fase calculado en double→float, la acumulación `float` de ángulo de `CreateRandomStarShapedPolygon`, el escalado por límite de altura y el caso ártico persistido (RMAP-010). Temperate y los fixtures árticos cubiertos ya son exactos; el pase y tipos de árbol tropicales siguen en RMAP-011, además de las entradas de clear/pueblos/industrias que divergen en el generador completo. No se cierra por una frontera de clima aislada. |
...
| **RMAP-011** | Árboles tropicales: zona, tipo y pase de rainforest. | **Abierto (P1)** | El primer fixture aislado `--climate tropic --size 64 --seed 1330935378` diverge en 550/4096 teselas y 147/256 bloques 4×4; la traza C++ tiene 625 llamadas admitidas por sustrato frente a 516 escrituras observadas en Rust. Faltan `GetTropicZone` (bits bajos de MAPT), `GetRandomTreeType` por zona (normal/rainforest/cactus e inválido en desierto), el pase `DEFAULT_RAINFOREST_TREE_STEPS` y hacer que la traza del candidato conserve los intentos `PlaceTree` que son no-op en desierto. Mantener abierto hasta cero diferencias de bytes y de traza en más de una semilla/tamaño. |
