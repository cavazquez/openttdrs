...
| **RMAP-004** | Reproducir el mapa aleatorio de OpenTTD con el mismo seed desde el generador procedural Rust. | **Abierto (P1, avance parcial)** | La portabilidad de TGP/RNG, costas, `water_borders`, bordes `MP_VOID`, conteo de ríos y la etapa pueblos/industrias ya está en el generador. En la corrida local de 15 casos, el cargador sigue exacto 15/15; el generador mismo-seed queda 0/15 exacto, con 3.846–260.108 teselas distintas por caso y todos los bloques 4×4 afectados. La densidad inicial `MakeClear(..., 3)` y el punto de observación de 1280 ciclos `RunTileLoop` ya están alineados en el harness; una sonda 64×64 bajó 3121→2073 teselas distintas, pero no es paridad. El residual es la secuencia/algoritmo de ríos y la paridad de `GenerateTowns`, `GenerateIndustries`, `GenerateObjects`, `GenerateTrees` y los campos `m1..m8`; no se cierra con ajustes de renderer o del lector SAV. |
