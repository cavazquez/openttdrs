...
| **RMAP-004** | Reproducir el mapa aleatorio de OpenTTD con el mismo seed desde el generador procedural Rust. | **Abierto (P1, avance parcial)** | La portabilidad de TGP/RNG, costas, `water_borders`, bordes `MP_VOID`, conteo de ríos y la etapa pueblos/industrias ya está en el generador. El port de `GenerateClearTile` ahora materializa rough/rocks con su recorrido diagonal y `m5`; el port de `GenerateTrees` usa el `Randomizer` de OpenTTD, grupos, `PlaceTree`, extras por altura y el layout `m1..m5`, y se ejecuta después de industrias; sus pruebas de contrato pasan. En la corrida local de 15 casos, el cargador sigue exacto 15/15; el generador mismo-seed queda 0/15 exacto, con 3.846–260.108 teselas distintas por caso y todos los bloques 4×4 afectados. La sonda 64×64 semilla `1330928978` queda en 2.230 diferencias tras ambas etapas (la referencia tiene 36 rough/53 rocky y 386 tiles de árboles; el candidato 72 rough/49 rocky y 458 árboles), por lo que los cambios corrigen contratos de bytes pero no la ubicación global. La densidad inicial `MakeClear(..., 3)` y el punto de observación de 1280 ciclos `RunTileLoop` ya están alineados en el harness; el residual es compartir la secuencia RNG real entre TGP, `GenerateClearTile`, pueblos, industrias, objetos y árboles, además de `GenerateObjects` y zonas tropicales. No se cierra con ajustes de renderer o del lector SAV. |
