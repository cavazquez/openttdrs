...
| Objetos | 🟡 lee las filas base de `OBJS` (ObjectID, ubicación, huella, town, fecha, color, vista y tipo), además de usar el pool para traducir tipos del mapa | 🟡 conserva `OBJS`/`OBID` sin cambios mientras no se muten objetos; tras construir/demoler reconstruye las filas base de `OBJS` y mantiene `OBID` opaco | Las columnas futuras de `OBJS`, el mapping `OBID` y el runtime completo de specs/callbacks de objetos siguen pendientes |
...
filas base; `OBID` permanece opaco. Las demás tablas reconstruidas (`VEHS`, `STNN`,
