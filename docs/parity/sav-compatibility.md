...
`old_max`/`new_max`/`old_act`/`new_act` por efecto) en `Town`. El encoder
canónico las reemite con los tamaños nativos (incluido el slot `TAE_NONE`),
pero estas series todavía no alimentan el crecimiento/economía; una carga sin
mutaciones sigue usando el cuerpo original por passthrough.
