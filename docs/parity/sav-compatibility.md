...
| Mundo base | ✅ ciudades, estaciones/waypoints, industrias, fecha y primera compañía best-effort; `INDY.accepted[].waiting`, producciones adicionales, layout seleccionado/random y metadata de fundador, fechas, tipo, flags y último año de producción se hidratan en la entidad NewGRF | ✅ `CITY`, `STNN`, `INDY` (incluidas listas anidadas `accepted`/`produced`, `selected_layout`, `random`, fundador, fechas, tipo, flags y `last_prod_year`), `DATE`, `PLYR` básicos | Historiales anidados de industria, `last_accepted`, PSA y campos de industria aún no modelados siguen fuera de este subconjunto |
...
| NewGRF | ✅ lee `NGRF` como tabla y restaura archivo, GRFID, versión y hasta 128 parámetros activos; `ENGN`, `EIDS` y mappings no modelados siguen como chunks opacos; las colas `INDY.accepted`/`produced`, `selected_layout`, `random` y metadata de fundador/fechas/tipo/flags/año se hidratan para CB1/CB2 y scopes | ✅ reconstruye `NGRF` para entradas activas no estáticas, con el array fijo de 128 parámetros (`num_params` conserva la longitud usada); `INDY` emite layout/random y metadata de instancia además de las listas conocidas; MD5 y `palette` se emiten con valores neutros porque aún no son parte del modelo | Runtime/cargos custom, PSA, `last_accepted` e historiales anidados de industria continúan parciales; `OBJS`/`OBID` todavía son passthrough; los labels no representables en el catálogo fijo se omiten |
