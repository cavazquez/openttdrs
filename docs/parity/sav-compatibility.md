...
| Mundo base | ✅ ciudades, estaciones/waypoints, industrias, fecha y primera compañía best-effort; `CITY` importa ahora la metadata nativa de nombre (`townnamegrfid`, `townnametype`, `townnameparts`), flags, ratings, `have_ratings`, unwanted, metas, crecimiento, exclusividad, layout, estatuas, historial válido y texto de GameScript; `CITY.psa_list` conserva referencias a storages de pueblos y sus registros no nulos se hidratan por GRFID en el pueblo; `INDY.accepted[].waiting`, `last_accepted`, producciones adicionales, layout seleccionado/random y metadata de fundador, fechas, tipo, flags y último año de producción se hidratan en la entidad NewGRF; `valid_history` e historiales anidados por cargo se conservan para round-trip; las referencias `INDY.psa` y `STNN.normal.airport.psa` enlazan las filas `PSAC` y sus 256 registros | ✅ `CITY` (incluido `psa_list`), `STNN`, `INDY` (incluidas listas anidadas `accepted`/`produced`, `selected_layout`, `random`, fundador, fechas, tipo, flags, `last_prod_year`, `valid_history`, historiales por cargo, `accepted[].accumulated_waiting`, `accepted[].last_accepted` y `psa`), `PSAC` para storages persistentes conservados o escritos por industrias, aeropuertos y pueblos, `DATE`, `PLYR` básicos | La metadata nativa de `CITY` recién importada alimenta el runtime y los scopes NewGRF, pero el writer canónico todavía emite el subconjunto mínimo y no hace writeback de esos campos ni de las listas/estructuras anidadas; el passthrough de un `CITY` sin cambios conserva el cuerpo original. La lectura runtime de `7C` de pueblo ya está disponible en los scopes parent de casas y objetos que tienen asociación a un pueblo; todavía falta writeback/mutación de esos registros y los scopes de otras entidades; campos no modelados continúan opacos |
...
Nota de corte 2026-09-02: `CITY` ya decodifica sus escalares nativos y los
expone al modelo `Town`; esto no convierte todavía al writer en un exportador
completo de ciudades. El pool `PSAC` de industria, aeropuerto y referencias
