...
| Compañías y noticias | 🟡 dinero/préstamo/meses de bancarrota/color/nombre/presidente/`face`/`face_style`/indicador AI, `settings.*` y las 23 entradas `PLYR.liveries` | 🟡 `PLYR` con esos campos, incluidas las libreas nativas (SLV355) | Faltan historial económico, límite especial de préstamo y flags completos; el renderer aún no aplica las libreas por tipo. La cola propia completa queda en JSON |
