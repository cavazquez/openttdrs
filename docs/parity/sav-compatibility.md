...
| Compañías y noticias | 🟡 dinero/préstamo/límite de préstamo individual (`PLYR.max_loan`, incluido el centinela global), saldo fraccional (`money_fraction`), bloqueo de preview (`block_preview`), meses de bancarrota/color/nombre/presidente/`face`/`face_style`/indicador AI, `settings.*`, 23 `PLYR.liveries` e historial trimestral (`cur_economy` + hasta 24 `old_economy`, incluido `delivered_cargo`) | 🟡 `PLYR` con esos campos, incluidas las libreas nativas (SLV355) y el orden más-reciente-primero de `old_economy`; un override de préstamo no es reemplazado por inflación | El core conserva `money_fraction`/`block_preview`, pero no ejecuta todavía sus reglas nativas. Faltan HQ, bancarrota/gastos/límites de compañía y otros flags; la cola propia completa queda en JSON y los consumidores de noticias siguen fuera del formato nativo. Ver [`sav-company-preview-375.md`](sav-company-preview-375.md) |
