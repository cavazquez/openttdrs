...
| Shared orders | 🟡 conserva el contenido; todavía no reconstruye `shared_order_id` al importar | 🟡 reutiliza una única `ORDL` para vehículos que comparten lista | La identidad lógica compartida no se vuelve a hidratar como pool propio |
...
| Ajustes | 🟡 lee el subconjunto ejecutado por el core de `PATS`/`OPTS`: construcción, pathfinding, averías, subsidios, desastres, autoridad, inflación/recesiones y unidades de tiempo | 🟡 escribe ese mismo subconjunto en `PATS`; siguen fuera `GSET`/`ENGN`/`SRND` completos | [`sav/settings.rs`](../../crates/openttdrs-core/src/sav/settings.rs), [`sav/landscape.rs`](../../crates/openttdrs-core/src/sav/landscape.rs) |
