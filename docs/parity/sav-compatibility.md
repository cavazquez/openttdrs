...
| Compañías y noticias | 🟡 dinero/color/nombre/indicador AI, `settings.*` y las 23 entradas `PLYR.liveries` | 🟡 `PLYR` con esos campos, incluidas las libreas nativas | Faltan historial económico, rostro y flags completos; el renderer aún no aplica las libreas por tipo. La cola propia completa queda en JSON |
