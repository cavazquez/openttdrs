...
`main` posterior al bloque de metadata nativa de industrias; referencia: **OpenTTD
...
| Mundo base | ✅ ciudades, estaciones/waypoints, industrias, fecha y primera compañía best-effort; `INDY.accepted[].waiting`, `last_accepted`, producciones adicionales, layout seleccionado/random y metadata de fundador, fechas, tipo, flags y último año de producción se hidratan en la entidad NewGRF; `valid_history` e historiales anidados por cargo se conservan para round-trip; las referencias `INDY.psa` enlazan las filas `PSAC` y sus 256 registros | ✅ `CITY`, `STNN`, `INDY` (incluidas listas anidadas `accepted`/`produced`, `selected_layout`, `random`, fundador, fechas, tipo, flags, `last_prod_year`, `valid_history`, historiales por cargo, `accepted[].accumulated_waiting`, `accepted[].last_accepted` y `psa`), `PSAC` para storages persistentes conservados o escritos por industrias, `DATE`, `PLYR` básicos | Storages PSA de estaciones/casas/objetos aún no tienen runtime propio; campos no modelados siguen fuera de este subconjunto |
...
| NewGRF | ✅ lee `NGRF` como tabla y restaura archivo, GRFID, versión y hasta 128 parámetros activos; `ENGN`, `EIDS` y mappings no modelados siguen como chunks opacos; las colas `INDY.accepted`/`produced`, `accepted[].last_accepted`, `selected_layout`, `random`, metadata de fundador/fechas/tipo/flags/año y `INDY.psa`→`PSAC` se hidratan para CB1/CB2 y scopes; los historiales mensuales por cargo, `accepted[].accumulated_waiting` y `valid_history` se conservan para round-trip | ✅ reconstruye `NGRF` para entradas activas no estáticas, con el array fijo de 128 parámetros (`num_params` conserva la longitud usada); `INDY` emite listas, historiales anidados, `valid_history`, layout/random, metadata de instancia, última aceptación por cargo y referencias PSA; `PSAC` reemite el pool con 256 registros por fila y conserva storages ajenos a industrias | Runtime/cargos custom, storages PSA de otras entidades y writeback de historiales después de mutaciones económicas continúan parciales; `OBJS`/`OBID` todavía son passthrough; los labels no representables en el catálogo fijo se omiten |

Nota de corte 2026-09-02: el pool `PSAC` de industria dejó de ser sólo
passthrough. `INDY.psa` se resuelve contra sus filas, los registros no nulos
se hidratan en `Industry.newgrf_persistent_regs` y el exportador reemite 256
valores por fila manteniendo índices y storages de entidades aún no modeladas.
La misma ruta conserva ahora `INDY.valid_history`, `accepted[].accumulated_waiting`
y las listas `history` de cada cargo; una mutación económica futura deberá
invalidar este snapshot para recalcularlas.
