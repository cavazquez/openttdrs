...
| Mundo base | ✅ ciudades, estaciones/waypoints, industrias, fecha y primera compañía best-effort; `INDY.accepted[].waiting` y producciones adicionales se hidratan en las colas NewGRF | ✅ `CITY`, `STNN`, `INDY`, `DATE`, `PLYR` básicos | El exportador `INDY` aún no serializa las listas anidadas `accepted`/`produced`; al volver a guardar se pierde esa cola nativa |
