...
| Mundo base | ✅ ciudades, estaciones/waypoints, industrias, fecha y primera compañía best-effort; `INDY.accepted[].waiting`, `last_accepted`, producciones adicionales, layout seleccionado/random y metadata de fundador, fechas, tipo, flags y último año de producción se hidratan en la entidad NewGRF; las referencias `INDY.psa` enlazan las filas `PSAC` y sus 256 registros | ✅ `CITY`, `STNN`, `INDY` (incluidas listas anidadas `accepted`/`produced`, `selected_layout`, `random`, fundador, fechas, tipo, flags, `last_prod_year`, `accepted[].last_accepted` y `psa`), `PSAC` para storages persistentes conservados o escritos por industrias, `DATE`, `PLYR` básicos | Storages PSA de estaciones/casas/objetos aún no tienen runtime propio; historiales anidados de industria y campos no modelados siguen fuera de este subconjunto |
...
| NewGRF | ✅ lee `NGRF` como tabla y restaura archivo, GRFID, versión y hasta 128 parámetros activos; `ENGN`, `EIDS` y mappings no modelados siguen como chunks opacos; las colas `INDY.accepted`/`produced`, `accepted[].last_accepted`, `selected_layout`, `random`, metadata de fundador/fechas/tipo/flags/año y `INDY.psa`→`PSAC` se hidratan para CB1/CB2 y scopes | ✅ reconstruye `NGRF` para entradas activas no estáticas, con el array fijo de 128 parámetros (`num_params` conserva la longitud usada); `INDY` emite layout/random, metadata de instancia, última aceptación por cargo y referencias PSA; `PSAC` reemite el pool con 256 registros por fila y conserva storages ajenos a industrias | Runtime/cargos custom, storages PSA de otras entidades e historiales anidados de industria continúan parciales; `OBJS`/`OBID` todavía son passthrough; los labels no representables en el catálogo fijo se omiten |
