...
| Órdenes | 🟡 estación, waypoint, depósito, condicionales, refit vanilla y flags soportados | 🟡 mismo subconjunto, una lista `ORDL` por vehículo y `StationID` nativo cuando proviene de un `.sav` | El refit vanilla de depósito (`0..10`) se restaura; cargos NewGRF, destinos/contextos no soportados y variantes avanzadas se degradan; estaciones nuevas sin ID importado usan índice denso como fallback |
