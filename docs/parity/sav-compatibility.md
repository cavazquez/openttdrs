...
Actualización #381 (2026-09-05): `PLYR.allow_list[].key` se conserva como el
struct-list moderno de claves públicas autorizadas. El header y las strings
anidadas ya cargan/re-guardan en OpenTTD 15.3; no implica que el runtime propio
...
reciba carga. El snapshot de `PATS` detecta también una mutación posterior a
importar; la evidencia está en
[`sav-order-selectgoods-382.md`](sav-order-selectgoods-382.md).

Actualización #383 (2026-09-05): `PATS.linkgraph.*` conserva ahora sus
segundos nativos, sus cuatro modos por clase y los knobs que el pipeline ya
ejecuta. El scheduler divide el intervalo por los dos segundos económicos
nativos, y los cargos NewGRF toman su clase de `CargoSpecDef`; `recalc_time`
se conserva pero no convierte el pipeline síncrono en el job/thread nativo.
La evidencia y los límites están en
[`sav-linkgraph-settings-383.md`](sav-linkgraph-settings-383.md).

Actualización #394 (2026-09-06): el scheduler determinista respeta ahora la
latencia observable de `PATS.linkgraph.recalc_time`. En el spawn se clona el
...
`Command::JoinStations` lo ejecuta para permitir o rechazar una unión no
adyacente sin relajar owner, tipo o eje rail. El dedicated OpenTTD 15.3
...
Actualización #386 (2026-09-06): `PATS.vehicle.disable_elrails` se importa y
emite como `SLE_BOOL`, con default nativo `false`. Al activarlo, las
locomotoras eléctricas pueden comprarse sobre rail normal y la conversión
Electric → Rail existente se vuelve no-op, siguiendo la actualización de
railtypes de OpenTTD. La catenaria visual global todavía depende del renderer;
ver [evidencia #386](sav-disable-elrails-386.md).

Actualización #387 (2026-09-06): `PATS.vehicle.plane_crashes` se importa y
emite como `SLE_UINT8`, con default nativo `2` y rango `0..=2`. El camino FTA
de accidentes aplica `0`/`1`/`2` con el cálculo de probabilidad de OpenTTD y
mantiene separado el caso fijo de jet en pista corta. Las rutas de accidente
que todavía no existen en el runtime propio y la UI nativa siguen pendientes;
ver [evidencia #387](sav-plane-crashes-387.md).

Actualización #388 (2026-09-06): `PATS.vehicle.plane_speed` se importa y
emite como `SLE_UINT8`, con default nativo `4` y rango `1..=4`. El movimiento
lineal y el motor FTA reciben ahora el divisor desde `GameState`; las APIs
históricas conservan el default `4`. La aceleración detallada, el callback
NewGRF `0x10`, la API de scripts y otras fórmulas de velocidad siguen siendo
residuales del runtime global; ver [evidencia #388](sav-plane-speed-388.md).
...
| Ajustes | 🟡 lee el subconjunto ejecutado por el core de `PATS`/`OPTS`: construcción, pathfinding, aceleración de trenes **y carretera**, `selectgoods`, unión distante de estaciones, `wagon_speed_limits`, `disable_elrails`, `plane_speed`, `plane_crashes`, perfil `linkgraph` por clase, averías, subsidios, desastres, autoridad, inflación/recesiones y unidades de tiempo | 🟡 escribe ese subconjunto en `PATS` y conserva `GSET`/`ENGN`/`SRND` nativos como passthrough | `linkgraph.recalc_time` no controla aún jobs síncronos/threads/pausa; la catenaria visual de `disable_elrails`, rutas de accidente fuera de FTA, las fórmulas completas de velocidad/FTA de aeronaves, los comandos completos de estación, settings restantes y UI nativa siguen parciales. [`sav/settings.rs`](../../crates/openttdrs-core/src/sav/settings.rs), [`sav/landscape.rs`](../../crates/openttdrs-core/src/sav/landscape.rs), [#382](sav-order-selectgoods-382.md), [#383](sav-linkgraph-settings-383.md), [#384](sav-distant-join-stations-384.md), [#385](sav-wagon-speed-limits-385.md), [#386](sav-disable-elrails-386.md), [#387](sav-plane-crashes-387.md), [#388](sav-plane-speed-388.md) |
| Compañías y noticias | 🟡 dinero/préstamo/límite de préstamo individual (`PLYR.max_loan`, incluido el centinela global), saldo fraccional (`money_fraction`), bloqueo de preview (`block_preview`), `TileIndex` de HQ/última construcción, años de inauguración económico/wallclock, estado pasivo de bancarrota, cupos de paisajismo 16.16, gastos anuales 3×13, `allow_list[].key`, meses de bancarrota/color/nombre/presidente/`face`/`face_style`/indicador AI, `settings.*`, 23 `PLYR.liveries` e historial trimestral (`cur_economy` + hasta 24 `old_economy`, incluido `delivered_cargo`) | 🟡 `PLYR` con esos campos, incluidas las claves autorizadas como struct-list, las libreas nativas (SLV355) y el orden más-reciente-primero de `old_economy`; un override de préstamo no es reemplazado por inflación | El core conserva los bytes de preview, inauguración, ubicación, bancarrota pasiva, cupos, gastos anuales y claves autorizadas, pero no ejecuta todavía sus reglas nativas ni los asigna/actualiza al crear empresas o construir. Faltan autenticación/red de `allow_list`, semántica/render de HQ, negociación/adquisición por bancarrota, descuento/recarga de cupos, cálculo/rotación de gastos y otros flags; la cola propia completa queda en JSON y los consumidores de noticias siguen fuera del formato nativo. Ver [`sav-company-preview-375.md`](sav-company-preview-375.md), [`sav-company-inauguration-376.md`](sav-company-inauguration-376.md), [`sav-company-location-377.md`](sav-company-location-377.md), [`sav-company-bankruptcy-378.md`](sav-company-bankruptcy-378.md), [`sav-company-landscaping-379.md`](sav-company-landscaping-379.md), [`sav-company-yearly-expenses-380.md`](sav-company-yearly-expenses-380.md) y [`sav-company-allow-list-381.md`](sav-company-allow-list-381.md) |
...
Actualización 2026-09-12 (`f443f713`): el lector de `ENGN` también resuelve los
slots vanilla de X2001/monorail (`54`) y Lev1/maglev (`84`). La creación de un
`ENGN` nuevo cuando el origen no lo contiene continúa pendiente porque el
formato denso requiere materializar el prefijo completo del pool para no
sobrescribir motores no representados.
