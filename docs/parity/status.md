...
| Entrada a playa de carga (bahía) | `station.rs` (`resolve_order_destination` → bahía, `is_connected_bay_road_stop`), `road_movement.rs` (`BAY_STOP_PROGRESS`, `bay_turn_subtile`) | `roadveh_cmd.cpp:1496-1502`, `roadveh_movement.h:1087` (`_road_stop_stop_frame`) | 3 · validado (Fase 2: entra a la tesela y carga dentro; el punto de parada visual aproxima el stop frame, sin las trayectorias `_rv_station_*` exactas) | `divergences_found.md` (`bay_stop_position` «no observada»); tests `truck_unloads_from_road_tile_adjacent_to_stop`, `parity_system.rs` | Medio: trayectoria interna aproximada (carril recto, no el lazo de `_rv_station_*`) |
