#!/usr/bin/env bash
# Ejecuta OpenTTD parcheado y exporta el oráculo JSONL world-semantic (#306).
set -euo pipefail
...
REGION="${4:-${OPENTTDRS_WORLD_SEMANTIC_REGION:-}}"
...
export OPENTTDRS_WORLD_SEMANTIC_OUT="$OUT"
export OPENTTDRS_WORLD_SEMANTIC_SOURCE="$SAV"
export OPENTTDRS_WORLD_SEMANTIC_SAVE_SHA256="$SAVE_SHA256"
export OPENTTDRS_OPENTTD_COMMIT="$COMMIT"
# Dedicated + -g carga antes una partida nueva; exportar en el segundo AfterLoadGame.
export OPENTTDRS_WORLD_SEMANTIC_MIN_CALL="${OPENTTDRS_WORLD_SEMANTIC_MIN_CALL:-${OPENTTDRS_WORLD_RAW_MIN_CALL:-${OPENTTDRS_SNAPSHOT_MIN_CALL:-2}}}"
if [[ -n "$REGION" ]]; then
  export OPENTTDRS_WORLD_SEMANTIC_REGION="$REGION"
fi

echo "world-semantic OpenTTD: bin=$BIN sav=$SAV out=$OUT region=${REGION:-full} commit=$COMMIT"
cd "$BUILD_DIR"
set +e
timeout 120s "$BIN" -X -I opengfx -D -g "$SAV" >/tmp/openttdrs-world-semantic-run.log 2>&1
rc=$?
set -e

if [[ ! -f "$OUT" ]]; then
  echo "error: no se generó $OUT (exit=$rc). Log:" >&2
  tail -n 40 /tmp/openttdrs-world-semantic-run.log >&2 || true
  exit 1
fi

if ! python3 -c "import json,sys; d=json.loads(open(sys.argv[1], encoding='utf-8').readline()); sys.exit(0 if d.get('producer') == 'openttd' and d.get('contract') == 'world-semantic' else 1)" "$OUT"; then
  echo "error: $OUT no declara metadata world-semantic de OpenTTD" >&2
  exit 1
fi

echo "OK: oráculo world-semantic escrito en $OUT (producer=openttd)"
