...
Layout (20×17, origen arriba-izquierda):
...
y=11  · mar · costa · recta Y NE · cruce SE/SW/NW en pendiente ·
y=12  · (buffer hierba bajo pendientes y=11) ·
y=13  · vía T (0x07) en pendiente NE · SE · SW · NW ·
y=14  · (buffer hierba) ·
y=15  · cruce X|Y (0x03) en pendiente NE · SE · SW · NW ·
y=16  · (buffer hierba) ·
```
...
def main() -> None:
    ...
    # --- SP3.2c: cruce X|Y en pendiente (y=15; comparar con cruce plano 7,5) ---
    for tx, ty, slope_fn in [
        (1, 15, apply_ne_slope),
        (4, 15, apply_se_slope),
        (7, 15, apply_sw_slope),
        (10, 15, apply_nw_slope),
    ]:
        slope_fn(tiles, tx, ty)
        cur = tiles.get((tx, ty), TileSpec())
